...
/* Procedure structure for procedure `GET_NON_BILLABLE` */

/*!50003 DROP PROCEDURE IF EXISTS  `GET_NON_BILLABLE` */;

DELIMITER $$

/*!50003 */
CREATE DEFINER=`root`@`%` PROCEDURE `GET_NON_BILLABLE`(in start_date DATE, in end_date DATE)
BEGIN 	
	    DECLARE i_employee_id int(10);
	    DECLARE i_alloc_start_date date;
	    DECLARE i_alloc_end_date date;
	    DECLARE i_allocation_type_id int(10);
	    DECLARE p_alloc_type int(10) DEFAULT 0;
	    DECLARE p_employee_id INT(10) DEFAULT 0;
	    DECLARE p_alloc_start_date DATE;
	    DECLARE p_alloc_end_date DATE;
	    DECLARE i_resource_name varchar(250);
	    DECLARE allocation_counter int(10) default 0; 
	    declare billable_days int(10) default 0;
	    DECLARE non_billable_days INT(10) DEFAULT 0;
	    DECLARE billable_allocation_counter INT(10) DEFAULT 0;
	    DECLARE days_count INT(10) DEFAULT 0;
	    DECLARE loop_couunter int(10) default 0;
	    
	    DECLARE cur_resource_allocation CURSOR FOR 
		SELECT resal.employee_id, concat(res.first_name, ' ', res.last_name) res_name ,resal.alloc_start_date,resal.alloc_end_date, 
		(case when resal.allocation_type_id =2 OR resal.allocation_type_id = 3 then 2 else 1 end) allocation_type
		FROM resource_allocation resal, resource res where resal.alloc_start_date>=start_date  
		AND res.employee_id = resal.employee_id ORDER BY resal.employee_id, resal.alloc_start_date;
		
	 DROP TEMPORARY TABLE IF EXISTS resource_allocation_report;
	 CREATE TEMPORARY TABLE resource_allocation_report(employee_id INT, employee_name VARCHAR(250),
		non_billable_days INT, billable_days INT, billable_allocation_counter INT);
	    
	    open cur_resource_allocation;
		igmLoop: loop
		fetch cur_resource_allocation into i_employee_id, i_resource_name ,i_alloc_start_date,i_alloc_end_date,i_allocation_type_id;
		BEGIN		
			if p_employee_id != i_employee_id AND loop_couunter <> 0 then 
				insert into resource_allocation_report values(p_employee_id,'',non_billable_days,billable_days,billable_allocation_counter);
				SET non_billable_days = 0;
				SET billable_days = 0;
				SET billable_allocation_counter = 0;
			end if;
			SET loop_couunter = loop_couunter+1;
			SET p_employee_id = i_employee_id;
			IF i_allocation_type_id = 2 THEN
				set billable_allocation_counter = billable_allocation_counter+1;
				SELECT DATEDIFF(i_alloc_end_date, i_alloc_start_date) INTO days_count;
				SET billable_days = billable_days+days_count;
			ELSE
				SELECT DATEDIFF(i_alloc_end_date, i_alloc_start_date) INTO days_count;
				SET non_billable_days = non_billable_days+days_count;
			END IF;	
		END;
	    end loop igmLoop;
	    close cur_resource_allocation;
	    INSERT INTO resource_allocation_report VALUES(p_employee_id,'', non_billable_days , billable_days,billable_allocation_counter );
	    SELECT * FROM resource_allocation_report;
    END */$$
DELIMITER ;

/* Procedure structure for procedure `GET_RESOURCE` */

/*!50003 DROP PROCEDURE IF EXISTS  `GET_RESOURCE` */;

DELIMITER $$

/*!50003 */ 
CREATE DEFINER=`root`@`%` PROCEDURE `GET_RESOURCE`()
BEGIN
...
/*!50003 */
CREATE DEFINER=`root`@`localhost` PROCEDURE `prepare_project_data_second`()
BEGIN
...
/*!50003 */
CREATE DEFINER=`root`@`%` PROCEDURE `prepare_project_widget_data`()
BEGIN
...
/*!50003 */ 
 CREATE DEFINER=`root`@`%` PROCEDURE `RMG_Bench_Grade_DaysWise_Report`(IN start_date CHAR(2), 
...
SET @STARTDATE = CONCAT(`start_year`,'-',`start_month`,'-',`start_date`);
SET @ENDDATE = CONCAT(`end_year`,'-',`end_month`,'-',`end_date`);
SELECT Y1.grade, COALESCE(0_to_1_month_on_Bench,0) '0 to 1 Month on Bench', COALESCE(1_to_2_month_on_Bench,0) '1 to 2 Month on Bench',
  COALESCE(2_to_3_month_on_Bench,0) '2 to 3 Month on Bench', COALESCE(More_than_3_Months_on_Bench,0) 'More than 3 Months on Bench' FROM
...
(SELECT grade,COUNT(A.employee_id) '0_to_1_month_on_Bench'  FROM
 ...
 FROM resource r1, resource_allocation r, grade g, allocationtype a
 WHERE r.employee_id=r1.employee_id AND r1.grade_id=g.id AND r.allocation_type_id=a.id 
 ...
 OR (alloc_end_date BETWEEN STR_TO_DATE(@STARTDATE, '%Y-%m-%d') AND STR_TO_DATE(@ENDDATE, '%Y-%m-%d')))
 AND allocation_old LIKE '%BENCH%'
 GROUP BY employee_id) A WHERE No_of_Days>0 AND No_of_Days<=30 GROUP BY grade) X1
...
(SELECT grade, COUNT(A.employee_id) '1_to_2_month_on_Bench' FROM
 ...
 OR (alloc_end_date BETWEEN STR_TO_DATE(@STARTDATE, '%Y-%m-%d') AND STR_TO_DATE(@ENDDATE, '%Y-%m-%d')))
 AND allocation_old LIKE '%BENCH%'
 GROUP BY employee_id) A WHERE No_of_Days>30 AND No_of_Days<=60 GROUP BY grade) Y
...
(SELECT grade,COUNT(A.employee_id) '2_to_3_month_on_Bench' FROM
 ...
 (ISNULL(alloc_end_date) OR alloc_end_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d')
 OR (alloc_end_date BETWEEN STR_TO_DATE(@STARTDATE, '%Y-%m-%d') AND STR_TO_DATE(@ENDDATE, '%Y-%m-%d')))
 AND allocation_old LIKE '%BENCH%' GROUP BY employee_id) A
...
/* Procedure structure for procedure `RMG_Bench_Grade_Report_old` */

/*!50003 DROP PROCEDURE IF EXISTS  `RMG_Bench_Grade_Report_old` */;

DELIMITER $$

/*!50003 */ 
CREATE DEFINER=`root`@`%` PROCEDURE `RMG_Bench_Grade_Report_old`(IN start_date CHAR(2), 
      IN end_date CHAR(2),IN start_month VARCHAR(2),
	  IN end_month VARCHAR(2),IN start_year CHAR(4), IN end_year VARCHAR(4))
BEGIN
SET @STARTDATE = CONCAT(`start_year`,'-',`start_month`,'-',`start_date`);
...
/*!50003 */
CREATE DEFINER=`root`@`%` PROCEDURE `RMG_BGBU_Metrics`(IN start_date CHAR(2), 
      IN end_date CHAR(2),IN start_month VARCHAR(2),
	  IN end_month VARCHAR(2),IN start_year CHAR(4), IN end_year VARCHAR(4))
BEGIN
SET @STARTDATE = CONCAT(`start_year`,'-',`start_month`,'-',`start_date`);
...
(ISNULL(r1.release_date) OR r1.release_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))
AND a.allocationtype LIKE '%Management%'  AND (ISNULL(alloc_end_date) OR alloc_end_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))
...
WHERE o1.id=o2.parent_id AND r1.current_bu_id=o2.id AND r.allocation_type_id=a.id AND r.employee_id=r1.employee_id AND
(ISNULL(r1.release_date) OR r1.release_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))
AND a.allocationtype LIKE 'Billable%' AND (ISNULL(alloc_end_date) OR alloc_end_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))
...
-- Below Query calculates unallocated resources for a BG-BU by following different criteria such as :
...
WHERE (ISNULL(r1.release_date) OR r1.release_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d')))
UNION
(SELECT DISTINCT o1.name BG, o2.name BU, r1.employee_id B1
FROM org_hierarchy o1 INNER JOIN org_hierarchy o2 ON o1.id=o2.parent_id
INNER JOIN resource r1 ON r1.current_bu_id=o2.id 
...
WHERE (ISNULL(r1.release_date) OR r1.release_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))
AND r.alloc_start_date < STR_TO_DATE(@STARTDATE, '%Y-%m-%d') AND r1.employee_id NOT IN
(SELECT DISTINCT r.employee_id B1
FROM resource_allocation r1 INNER JOIN resource r ON r1.employee_id=r.employee_id
WHERE (ISNULL(r1.release_date) OR r1.release_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d')) AND 
(ISNULL(alloc_end_date) OR alloc_end_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))))
) t4 
 GROUP BY t4.BG, t4.BU ) H
ON G.BG=H.BG AND G.BU=H.BU;
END $$
DELIMITER ;

/* Procedure structure for procedure `RMG_BGBU_Metrics_old` */

/*!50003 DROP PROCEDURE IF EXISTS  `RMG_BGBU_Metrics_old` */;

DELIMITER $$

/*!50003 */ 
CREATE DEFINER=`root`@`%` PROCEDURE `RMG_BGBU_Metrics_old`(IN start_date CHAR(2), 
      IN end_date CHAR(2),IN start_month VARCHAR(2),
	  IN end_month VARCHAR(2),IN start_year CHAR(4), IN end_year VARCHAR(4))
BEGIN
SET @STARTDATE = CONCAT(`start_year`,'-',`start_month`,'-',`start_date`);
...
GROUP BY o1.name,o2.name) A LEFT OUTER JOIN
(SELECT o1.name BG,o2.name BU,COUNT(r1.employee_id) B1
FROM org_hierarchy o1, org_hierarchy o2, resource_allocation r, allocationtype a,resource r1
...
(ISNULL(release_date) OR (YEAR(release_date)>=`end_year` AND MONTH(release_date)>=`end_month` 
AND DAY(release_date)>=`end_date`))
AND a.allocationtype LIKE 'Billable%' AND (ISNULL(alloc_end_date) OR (YEAR(alloc_end_date)>=`end_year` AND 
 MONTH(alloc_end_date)>=`end_month` AND DAY(alloc_end_date)>=`end_date`))
GROUP BY o1.name,o2.name)D
ON C.BG=D.BG AND C.BU=D.BU) E LEFT OUTER JOIN
(SELECT o1.name BG,o2.name BU,COUNT(DISTINCT(r1.employee_id)) B1
FROM org_hierarchy o1, org_hierarchy o2, resource_allocation r, allocationtype a,resource r1
...
AND a.allocationtype NOT LIKE 'Billable%' AND a.allocationtype NOT LIKE '%Management%' AND
a.allocationtype NOT LIKE '%Unallocated%' AND a.allocationtype NOT LIKE 'Non-Billable (Investment)'  
AND (ISNULL(alloc_end_date) OR (YEAR(alloc_end_date)>=`end_year` AND 
 MONTH(alloc_end_date)>=`end_month` AND DAY(alloc_end_date)>=`end_date`))
GROUP BY o1.name,o2.name)F
ON E.BG=F.BG AND E.BU=F.BU) G LEFT OUTER JOIN 
(SELECT o1.name BG,o2.name BU,COUNT(DISTINCT(r1.employee_id)) B1
...
AND DAY(release_date)>=`end_date`))
AND (a.allocationtype LIKE '%Unallocated%' OR a.allocationtype LIKE 'Non-Billable (Investment)') AND 
(ISNULL(alloc_end_date) OR (YEAR(alloc_end_date)>=`end_year` AND MONTH(alloc_end_date)>=`end_month` AND DAY(alloc_end_date)>=`end_date`))
...
/*!50003 */
CREATE DEFINER=`root`@`%` PROCEDURE `RMG_Projects_Closing_In_3_Months`(IN start_date CHAR(2), 
      IN end_date CHAR(2),IN start_month VARCHAR(2),
	  IN end_month VARCHAR(2),IN start_year CHAR(4), IN end_year VARCHAR(4))
BEGIN
SET @STARTDATE = CONCAT(`start_year`,'-',`start_month`,'-',`start_date`);
...
/*!50003*/ 
CREATE DEFINER=`root`@`%` PROCEDURE `RMG_Resource_Metrics`(IN start_date CHAR(2), 
      IN end_date CHAR(2),IN start_month VARCHAR(2),
	  IN end_month VARCHAR(2),IN start_year CHAR(4), IN end_year VARCHAR(4))
BEGIN
SET @STARTDATE = CONCAT(`start_year`,'-',`start_month`,'-',`start_date`);
...
/*!50003 */
CREATE DEFINER=`root`@`%` PROCEDURE `RMG_Resource_Metrics_byBG`(IN start_date CHAR(2), 
      IN end_date CHAR(2),IN start_month VARCHAR(2),
	  IN end_month VARCHAR(2),IN start_year CHAR(4), IN end_year VARCHAR(4))
BEGIN
...
/*!50003 */ 
CREATE DEFINER=`root`@`%` PROCEDURE `RMG_Resource_Metrics_byBGBU_Report`(IN start_date CHAR(2), 
      IN end_date CHAR(2),IN start_month VARCHAR(2),
	  IN end_month VARCHAR(2),IN start_year CHAR(4), IN end_year VARCHAR(4))
BEGIN
...
/*!50003 */
CREATE DEFINER=`root`@`%` PROCEDURE `RMG_Skills_Releasing_In_3_Months`(IN start_date CHAR(2), 
      IN end_date CHAR(2),IN start_month VARCHAR(2),
	  IN end_month VARCHAR(2),IN start_year CHAR(4), IN end_year VARCHAR(4))
BEGIN
SET @STARTDATE = CONCAT(`start_year`,'-',`start_month`,'-',`start_date`);
...
/*!50003 */
CREATE DEFINER=`root`@`%` PROCEDURE `RRF_Metrics`(IN start_date CHAR(2), 
      IN end_date CHAR(2),IN start_month VARCHAR(2),
	  IN end_month VARCHAR(2),IN start_year CHAR(4), IN end_year VARCHAR(4))
BEGIN
...
/*!50003 */
CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_rpt_Employee_Planed_Actual_Bill_hrs`()
BEGIN
...
/*!50003 */
CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_rpt_WeeklyTimeSheet`()
BEGIN 
...
/*!50003 */
CREATE DEFINER=`root`@`%` PROCEDURE `usp_set_current_project`()
BEGIN 
SET @EndDate=CURDATE();
...
/*!50003 */
CREATE DEFINER=`root`@`localhost` PROCEDURE `WeeklyTimeSheet`()
BEGIN 
...
-- The third query finds those resources who has assignedd default project. 
 ...
 WHERE o1.id=o2.parent_id AND r1.current_bu_id=o2.id AND r.employee_id=r1.employee_id AND 
 r.allocation_type_id=a.id AND (ISNULL(r1.release_date) OR r1.release_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))
 AND STR_TO_DATE(@ENDDATE, '%Y-%m-%d') >= alloc_start_date AND
 (ISNULL(alloc_end_date) OR alloc_end_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))
 AND allocation_old LIKE '%BENCH%'
...
DROP PROCEDURE IF EXISTS `RMG_Resource_Metrics_byBG`$$
...
(SELECT ABC.BG,ABC.A1 A1,ABC.B1 B1,ABC.C1 C1,D.D1 FROM
(SELECT AB.BG BG,AB.A1 A1,AB.B1 B1,C.C1 FROM 
(SELECT A.BG BG,A.A1 A1,B.B1 B1 FROM
(SELECT o1.name BG,COUNT(*) A1
FROM org_hierarchy o1, org_hierarchy o2, resource r
...
(SELECT o1.name BG,COUNT(*) C1
 FROM org_hierarchy o1, org_hierarchy o2, resource r
 WHERE o1.id=o2.parent_id AND r.current_bu_id=o2.id AND  date_of_joining 
...
GROUP BY o1.name) X LEFT OUTER JOIN 
 ...
 YEAR(release_date)>=`start_year` AND MONTH(release_date)>=`start_month` AND DAY(release_date)>=`start_date`
 AND YEAR(release_date)<=`end_year` AND MONTH(release_date)<=`end_month` AND DAY(release_date)<=`end_date`
 GROUP BY o1.name) Y ON X.BG=Y.BG) F
ON ABCDE.BG=F.BG) ABCDEF LEFT OUTER JOIN
(SELECT o1.name BG,'NA' G1
 FROM org_hierarchy o1, org_hierarchy o2, resource r
 WHERE o1.id=o2.parent_id AND r.current_bu_id=o2.id
...
(SELECT o1.name BG,COALESCE(COUNT(*),0) X1
 ...
 (ISNULL(ra.alloc_end_date) OR ra.alloc_end_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d')) AND
 a.allocationtype NOT LIKE '%Unallocated%' AND a.allocationtype NOT LIKE 'Non-Billable (Investment)' 
 GROUP BY o1.name) X LEFT OUTER JOIN
(SELECT o1.name BG, COALESCE(COUNT(*),0) Y1
FROM org_hierarchy o1, org_hierarchy o2, resource r
WHERE o1.id=o2.parent_id AND r.current_bu_id=o2.id AND r.date_of_joining <= STR_TO_DATE(@ENDDATE, '%Y-%m-%d') 
AND (ISNULL(r.release_date) OR r.release_date > STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))
GROUP BY o1.name) Y ON X.BG=Y.BG) I
ON ABCDEFGH.BG=I.BG;
...
SET @ENDDATE = CONCAT(`end_year`,'-',`end_month`,'-',`end_date`);
 ...
 SELECT 'Number of Active Resources' AS 'Resource Metrics', COUNT(*) AS ' '
 FROM resource WHERE release_date>STR_TO_DATE(@ENDDATE, '%Y-%m-%d') OR release_date IS NULL
 UNION
 SELECT CONCAT('Resources Joined in ',`start_year`),COUNT(*)
 FROM resource
 WHERE YEAR(date_of_joining) BETWEEN `start_year` AND `end_year`
 ...
 AND (DAY(release_date)>=`start_date` AND DAY(release_date)<=`end_date`)))/
 (SELECT COUNT(*) FROM resource 
   WHERE  release_date>STR_TO_DATE(@ENDDATE, '%Y-%m-%d') OR release_date IS NULL)*100),2),'%')
 FROM DUAL
 UNION
 SELECT 'Management Ratio', 'NA'
 FROM DUAL
 ...
 UNION
 SELECT '% Allocated',
 CONCAT(ROUND(((SELECT COUNT(DISTINCT(r1.employee_id))
  FROM org_hierarchy o1, org_hierarchy o2, resource r1, resource_allocation r, allocationtype a
  WHERE o1.id=o2.parent_id AND r1.current_bu_id=o2.id AND r.employee_id=r1.employee_id AND r.allocation_type_id=a.id AND
  (ISNULL(r1.release_date) OR r1.release_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))
  AND a.allocationtype NOT LIKE '%Unallocated%' AND a.allocationtype NOT LIKE 'Non-Billable (Investment)' 
  AND (ISNULL(alloc_end_date) OR alloc_end_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d')))/
  (SELECT COUNT(*) FROM resource WHERE release_date>STR_TO_DATE(@ENDDATE, '%Y-%m-%d') OR release_date IS NULL)*100),2),'%')
 FROM DUAL 
 UNION
 SELECT 'Average Cost', 'NA'
 FROM DUAL;
...
SET @ENDDATE = CONCAT(`end_year`,'-',`end_month`,'-',`end_date`);
 SELECT 'Number of Active Resources' AS 'Resource Metrics', COUNT(*) AS ' '
 FROM resource WHERE date_of_joining <= STR_TO_DATE(@ENDDATE, '%Y-%m-%d') AND 
...
SELECT '% Allocated',
 CONCAT(ROUND(((SELECT COUNT(DISTINCT(r1.employee_id))
  ...
  (ISNULL(r1.release_date) OR r1.release_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))
  AND a.allocationtype NOT LIKE '%Unallocated%' AND a.allocationtype NOT LIKE 'Non-Billable (Investment)' 
   AND alloc_start_date <= STR_TO_DATE(@ENDDATE, '%Y-%m-%d') AND r.curProj=1 AND
  (ISNULL(alloc_end_date) OR alloc_end_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d')))/
  (SELECT COUNT(employee_id) FROM resource WHERE `date_of_joining` <= STR_TO_DATE(@ENDDATE, '%Y-%m-%d') AND 
   (ISNULL(release_date) OR release_date>STR_TO_DATE(@ENDDATE, '%Y-%m-%d')))*100),2),'%')
 FROM DUAL 
 UNION
 SELECT 'Average Cost', 'NA'
 FROM DUAL;
...
DROP PROCEDURE IF EXISTS `RMG_Resource_Metrics_byBGBU_Report`$$
...
SET @STARTDATE = CONCAT(`start_year`,'-',`start_month`,'-',`start_date`);
SET @ENDDATE = CONCAT(`end_year`,'-',`end_month`,'-',`end_date`);
SELECT RTRIM(LTRIM(ABCDE.BG)) Parent_BG, RTRIM(LTRIM(ABCDE.BU)) Parent_BU, Number_of_Resources, COALESCE(ABCDE.B1,0) Joined_in_Year,
 COALESCE(ABCDE.C1,0) Joined_in_YearMonth, COALESCE(ABCDE.D1,0) Left_in_Year, COALESCE(Left_in_YearMonth,0) Left_in_YearMonth,
 CONCAT(ROUND((Left_in_YearMonth/Number_of_Resources*100),2),'%') YearMonth_Attrition FROM
 (SELECT ABCD.BG BG,ABCD.BU BU,ABCD.A1 Number_of_Resources,ABCD.B1 B1,ABCD.C1 C1,ABCD.D1 D1, E.E1 Left_in_YearMonth FROM
 ((SELECT ABC.BG BG,ABC.BU BU,ABC.A1 A1,ABC.B1 B1,ABC.C1,D.D1  FROM 
 (SELECT AB.BG BG,AB.BU BU,AB.A1 A1,AB.B1 B1,C.C1  FROM
 ((SELECT A.BG BG,A.BU BU,A.A1 A1,B.B1 B1 FROM 
 (SELECT o1.name BG,o2.name BU,COUNT(*) A1  FROM
...
SELECT BG,BU, Total, Joined_in_Year,Joined_in_YearMonth, Left_in_Year,
 Left_in_YearMonth, YearMonth_Attrition, CONCAT(ROUND((total_billable/billable_non_billable*100),2),'%') 'billable_non_billable',
 CONCAT(ROUND((total_billable/billable_non_billable_except_investment*100),2),'%') 'billable_non_billable_except_investment',
 CONCAT(ROUND((total_billable/billable_non_billable_except_investment_trainee*100),2),'%') 
...
(SELECT ABCDEFGH.BG BG, ABCDEFGH.BU BU, ABCDEFGH.Total, ABCDEFGH.Joined_in_Year, ABCDEFGH.Joined_in_YearMonth, ABCDEFGH.Left_in_Year,
 ABCDEFGH.Left_in_YearMonth, ABCDEFGH.YearMonth_Attrition, ABCDEFGH.total_billable,ABCDEFGH.billable_non_billable, 
 ABCDEFGH.billable_non_billable_except_investment,COALESCE(I.I1,0) AS 'billable_non_billable_except_investment_trainee' 
 FROM
(SELECT ABCDEFG.BG, ABCDEFG.BU, ABCDEFG.Total, ABCDEFG.Joined_in_Year,ABCDEFG.Joined_in_YearMonth, ABCDEFG.Left_in_Year,
 ABCDEFG.Left_in_YearMonth, ABCDEFG.YearMonth_Attrition, ABCDEFG.total_billable,ABCDEFG.billable_non_billable, 
 COALESCE(H.H1,0) AS 'billable_non_billable_except_investment'
 FROM
(SELECT ABCDEF.BG, ABCDEF.BU, ABCDEF.Total, ABCDEF.Joined_in_Year,ABCDEF.Joined_in_YearMonth, ABCDEF.Left_in_Year,
 ABCDEF.Left_in_YearMonth, ABCDEF.YearMonth_Attrition, ABCDEF.total_billable, COALESCE(G.G1,0) AS 'billable_non_billable'
 FROM
...
(SELECT o1.name BG,o2.name BU,COUNT(*) E1 FROM 
  ...
  LEFT OUTER JOIN
   ...
   -- Below query finds total active billable resources
   
  (SELECT o1.name BG,o2.name BU,COUNT(DISTINCT(r1.employee_id)) F1
  FROM org_hierarchy o1, org_hierarchy o2, resource r1, resource_allocation r, allocationtype a
  WHERE o1.id=o2.parent_id AND r1.current_bu_id=o2.id AND r.employee_id=r1.employee_id AND r.allocation_type_id=a.id 
  AND date_of_joining <= STR_TO_DATE(@ENDDATE, '%Y-%m-%d') AND
  (ISNULL(r1.release_date) OR r1.release_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d')) AND 
  a.allocationtype LIKE 'Billable%' AND alloc_start_date <= STR_TO_DATE(@ENDDATE, '%Y-%m-%d')
  AND (ISNULL(alloc_end_date) OR alloc_end_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d')) AND r.curProj=1
  GROUP BY o1.name,o2.name) F
  ON ABCDE.BG=F.BG AND ABCDE.BU=F.BU) ABCDEF
  
  LEFT OUTER JOIN
  
   -- Below query finds all billable and non billable (including all non-billable type) resources.
  ...
  AND date_of_joining <= STR_TO_DATE(@ENDDATE, '%Y-%m-%d') AND
  (ISNULL(r1.release_date) OR r1.release_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))
  AND (a.allocationtype LIKE 'Billable%' OR a.allocationtype LIKE 'Non-Billable%') 
  ...
  LEFT OUTER JOIN
   ...
   -- Below query finds all billable and non billable (excluding non-billable investment) resources.
  ...
  (SELECT o1.name BG,o2.name BU,COUNT(DISTINCT(r1.employee_id)) H1
   ...
   (ISNULL(r1.release_date) OR r1.release_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))
   AND (a.allocationtype LIKE 'Billable%' OR a.allocationtype LIKE 'Non-Billable%') 
   AND a.allocationtype NOT LIKE 'Non-Billable (Investment)' AND
   ...
   -- Below query finds all billable and non billable (excluding non-billable(investment) and non-billable(Trainee)) resources.
  ...
  (SELECT o1.name BG,o2.name BU,COUNT(DISTINCT(r1.employee_id)) I1
   ...
   AND date_of_joining <= STR_TO_DATE(@ENDDATE, '%Y-%m-%d') AND
   (ISNULL(r1.release_date) OR r1.release_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))
   AND (a.allocationtype LIKE 'Billable%' OR a.allocationtype LIKE 'Non-Billable%') AND
...
SET @STARTDATE = CONCAT(`start_year`,'-',`start_month`,'-',`start_date`);
SET @ENDDATE = CONCAT(`end_year`,'-',`end_month`,'-',`end_date`);
SELECT ABCDEFGH.BG 'BG',ABCDEFGH.BU 'BU',ABCDEFGH.A1 'Number of Resources',
COALESCE(ABCDEFGH.B1,0) 'Joined in Year',COALESCE(ABCDEFGH.C1,0) 'Joined in YearMonth',COALESCE(ABCDEFGH.D1,0) 'Left in Year',
COALESCE(ABCDEFGH.E1,0) 'Left in YearMonth',ABCDEFGH.F1 'YearMonth Attrition', ABCDEFGH.G1 'Management Ratio',
ABCDEFGH.H1 '% Billing' ,I.I1 '% Allocated' FROM 
(SELECT ABCDEFG.BG,ABCDEFG.BU,ABCDEFG.A1,ABCDEFG.B1,ABCDEFG.C1,ABCDEFG.D1,ABCDEFG.E1,ABCDEFG.F1,
      ABCDEFG.G1,H.H1 FROM 
(SELECT ABCDEF.BG,ABCDEF.BU,ABCDEF.A1,ABCDEF.B1,ABCDEF.C1,ABCDEF.D1,ABCDEF.E1,ABCDEF.F1,G.G1 FROM 
(SELECT ABCDE.BG,ABCDE.BU,ABCDE.A1,ABCDE.B1,ABCDE.C1,ABCDE.D1,ABCDE.E1,F.F1 FROM 
(SELECT ABCD.BG BG,ABCD.BU BU,ABCD.A1 A1,ABCD.B1 B1, ABCD.C1 C1,ABCD.D1 D1,E1 FROM 
(SELECT ABC.BG,ABC.BU,ABC.A1 A1,ABC.B1 B1,ABC.C1 C1,D.D1 FROM
(SELECT AB.BG BG,AB.BU BU,AB.A1 A1,AB.B1 B1,C.C1 FROM 
(SELECT A.BG BG,A.BU BU,A.A1 A1,B.B1 B1 FROM
(SELECT o1.name BG,o2.name BU,COUNT(*) A1
FROM org_hierarchy o1, org_hierarchy o2, resource r
...
(SELECT o1.name BG,o2.name BU ,COUNT(*) E1
 ...
 release_date >= STR_TO_DATE(@STARTDATE, '%Y-%m-%d') AND release_date <= STR_TO_DATE(@ENDDATE, '%Y-%m-%d')
 GROUP BY o1.name,o2.name) E
 ON ABCD.BG=E.BG AND ABCD.BU=E.BU) ABCDE LEFT OUTER JOIN
 
(SELECT X.BG, X.BU, CONCAT(ROUND(Y.Y1/X.X1*100,2),'%') F1 FROM  
 (SELECT o1.name BG, o2.name BU, COUNT(*) X1
FROM org_hierarchy o1, org_hierarchy o2, resource r
WHERE o1.id=o2.parent_id AND r.current_bu_id=o2.id AND r.date_of_joining <= STR_TO_DATE(@ENDDATE, '%Y-%m-%d') 
AND (ISNULL(r.release_date) OR r.release_date > STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))
GROUP BY o1.name,o2.name) X LEFT OUTER JOIN 
 (SELECT o1.name BG, o2.name BU, COUNT(*) Y1
 FROM org_hierarchy o1, org_hierarchy o2, resource r
 WHERE o1.id=o2.parent_id AND r.current_bu_id=o2.id AND 
 release_date >= STR_TO_DATE(@STARTDATE, '%Y-%m-%d') AND release_date <= STR_TO_DATE(@ENDDATE, '%Y-%m-%d')
 GROUP BY o1.name, o2.name) Y ON X.BG=Y.BG AND X.BU=Y.BU) F
ON ABCDE.BG=F.BG AND ABCDE.BU=F.BU) ABCDEF LEFT OUTER JOIN
(SELECT o1.name BG,o2.name BU,'NA' G1
 FROM org_hierarchy o1, org_hierarchy o2, resource r
 WHERE o1.id=o2.parent_id AND r.current_bu_id=o2.id
 GROUP BY o1.name,o2.name) G
ON ABCDEF.BG=G.BG AND ABCDEF.BU=G.BU) ABCDEFG LEFT OUTER JOIN
(SELECT o1.name BG,o2.name BU,'NA' H1
 FROM org_hierarchy o1, org_hierarchy o2, resource r
 WHERE o1.id=o2.parent_id AND r.current_bu_id=o2.id
 GROUP BY o1.name, o2.name) H
ON ABCDEFG.BG=H.BG AND ABCDEFG.BU=H.BU) ABCDEFGH LEFT OUTER JOIN
(SELECT X.BG BG,X.BU BU,CONCAT(ROUND(X.X1/Y.Y1*100,2),'%') I1 FROM 
(SELECT o1.name BG,o2.name BU,COALESCE(COUNT(*),0) X1
 FROM org_hierarchy o1, org_hierarchy o2, resource r ,resource_allocation ra, allocationtype a
 WHERE o1.id=o2.parent_id AND r.current_bu_id=o2.id AND r.employee_id=ra.employee_id AND
 ra.allocation_type_id=a.id AND r.date_of_joining <= STR_TO_DATE(@ENDDATE, '%Y-%m-%d') 
 AND (ISNULL(r.release_date) OR r.release_date > STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))
 ...
 a.allocationtype NOT LIKE '%Unallocated%' AND a.allocationtype NOT LIKE 'Non-Billable (Investment)' 
 GROUP BY o1.name,o2.name) X LEFT OUTER JOIN
(SELECT o1.name BG, o2.name BU ,COALESCE(COUNT(*),0) Y1
FROM org_hierarchy o1, org_hierarchy o2, resource r
WHERE o1.id=o2.parent_id AND r.current_bu_id=o2.id AND r.date_of_joining <= STR_TO_DATE(@ENDDATE, '%Y-%m-%d') 
AND (ISNULL(r.release_date) OR r.release_date > STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))
GROUP BY o1.name,o2.name) Y ON X.BG=Y.BG AND X.BU=Y.BU) I
ON ABCDEFGH.BG=I.BG AND ABCDEFGH.BU=I.BU;
END$$
...
SET @ENDDATE = CONCAT(`end_year`,'-',`end_month`,'-',`end_date`);
 ...
 CONCAT(ROUND(COALESCE(ABCDEF.E1/ABCDEF.A1*100,0),2),'%') 'year_month_attrition' FROM
(SELECT ABCDE.BG ,ABCDE.BU ,ABCDE.A1 ,ABCDE.B1,ABCDE.C1, ABCDE.D1, ABCDE.E1, F.F1 FROM
...
SELECT K.BG, K.BU,Total,Management_Resources,Billed_Resources,Unbilled_Resources,
        Unallocated_Resources, Billable_Percant , CONCAT(L.L1,'%') AS 'Un-Allocation_Percant',
        CONCAT(ROUND((((Management_Resources+Billed_Resources+Unbilled_Resources)/Total)*100),2),'%') 'Utilization_Percant'  FROM    
(SELECT I.BG, I.BU,Total,Management_Resources,Billed_Resources,Unbilled_Resources,
        Unallocated_Resources, CONCAT(COALESCE(J.J1,0),'%') AS 'Billable_Percant' FROM      
...
AND alloc_start_date <= STR_TO_DATE(@ENDDATE, '%Y-%m-%d') AND
(ISNULL(r1.release_date) OR r1.release_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d')) 
AND (a.allocationtype LIKE '%Unallocated%' OR a.allocationtype LIKE 'Non-Billable (Investment)')
AND (ISNULL(alloc_end_date) OR alloc_end_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d')))
UNION 
(SELECT DISTINCT o1.name BG,o2.name BU, r1.employee_id B1
FROM org_hierarchy o1 INNER JOIN org_hierarchy o2 ON o1.id=o2.parent_id
...
(SELECT X.BG BG,X.BU BU,ROUND(X.X1/Y.Y1*100,2) J1 FROM 
(SELECT Y.BG BG , Y.BU BU , SUM(Y.X1) X1 FROM
((SELECT o1.name BG,o2.name BU,COALESCE(COUNT(DISTINCT r.employee_id),0) X1
 ...
 UNION
 (SELECT o1.name BG,o2.name BU, COUNT(DISTINCT r.employee_id) X1
 FROM org_hierarchy o1, org_hierarchy o2, resource r ,resource_allocation ra, allocationtype a
 WHERE o1.id=o2.parent_id AND r.current_bu_id=o2.id AND r.employee_id=ra.employee_id AND
 ra.allocation_type_id=a.id AND r.date_of_joining <= STR_TO_DATE(@ENDDATE, '%Y-%m-%d') 
 AND (ISNULL(r.release_date) OR r.release_date > STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))
 ...
 GROUP BY Y.BG, Y.BU) X
 LEFT OUTER JOIN
 (SELECT o1.name BG, o2.name BU ,COALESCE(COUNT(r.employee_id),0) Y1
 FROM org_hierarchy o1, org_hierarchy o2, resource r
 WHERE o1.id=o2.parent_id AND r.current_bu_id=o2.id AND r.date_of_joining <= STR_TO_DATE(@ENDDATE, '%Y-%m-%d') 
 AND (ISNULL(r.release_date) OR r.release_date > STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))
 GROUP BY o1.name,o2.name) Y ON X.BG=Y.BG AND X.BU=Y.BU) J
 ON I.BG=J.BG AND I.BU=J.BU) K 
 
 LEFT OUTER JOIN
 
 -- Below query written to calculate percantage of un-allocated resource
 
 (SELECT Y.BG BG,Y.BU BU, COALESCE(ROUND(X.X1/Y.Y1*100,2),0) L1 FROM 
  
 (SELECT o1.name BG, o2.name BU ,COALESCE(COUNT(*),0) Y1
 FROM org_hierarchy o1, org_hierarchy o2, resource r
 WHERE o1.id=o2.parent_id AND r.current_bu_id=o2.id AND r.date_of_joining <= STR_TO_DATE(@ENDDATE, '%Y-%m-%d') 
 AND (ISNULL(r.release_date) OR r.release_date > STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))
 GROUP BY o1.name,o2.name) Y 
 LEFT OUTER JOIN
(SELECT DISTINCT t4.BG BG, t4.BU BU, COUNT(DISTINCT(B1)) X1 FROM (
(SELECT DISTINCT o1.name BG,o2.name BU, r1.employee_id B1
FROM org_hierarchy o1, org_hierarchy o2, resource_allocation r, allocationtype a,resource r1
WHERE o1.id=o2.parent_id AND r1.current_bu_id=o2.id AND r.allocation_type_id=a.id AND 
r.employee_id=r1.employee_id AND date_of_joining <= STR_TO_DATE(@ENDDATE, '%Y-%m-%d')
...
AND alloc_start_date <= STR_TO_DATE(@ENDDATE, '%Y-%m-%d') AND 
(ISNULL(alloc_end_date) OR alloc_end_date >= STR_TO_DATE(@ENDDATE, '%Y-%m-%d'))))) t4 
 GROUP BY t4.BG, t4.BU) X 
 ON Y.BG=X.BG AND Y.BU=X.BU) L
 ON K.BG=L.BG AND K.BU=L.BU ;
...
SELECT I.BG, I.BU,Total,Management_Resources,Billed_Resources,Unbilled_Resources,
        Unallocated_Resources, CONCAT(COALESCE(J.J1,0),'%') AS 'Billable_Percant',
       CONCAT(ROUND(COALESCE(Unallocated_Resources/Total*100,0),2),'%') AS 'Un-Allocation_Percant',
       CONCAT(ROUND((((Management_Resources+Billed_Resources+Unbilled_Resources)/Total)*100),2),'%') 'Utilization_Percant' FROM  
...
SET @ENDDATE = CONCAT(`end_year`,'-',`end_month`,'-',`end_date`);
 ...
 SELECT Q.BG BG, Q.BU BU,Total,Management_Resources,Billed_Resources,Unbilled_Resources,
  Unallocated_Resources, CONCAT(ROUND(COALESCE(Bill_partial_bill/Total*100,0),2),'%') AS 'Bill_Percant',  
  ...
  CONCAT(ROUND(COALESCE(Unallocated_Resources/Total*100,0),2),'%') AS 'Un-Allocation_Percant',
  CONCAT(ROUND((((Management_Resources+Billed_Resources+Unbilled_Resources)/Total)*100),2),'%') 'Utilization_Percant'
  FROM
