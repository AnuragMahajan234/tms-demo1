...
import org.yash.rms.report.dto.BorrowedResourcesPLReport;
import org.yash.rms.report.dto.ContractResourcePLReport;
import org.yash.rms.report.dto.PLReport;
import org.yash.rms.report.dto.ResignedResourcePLReport;
...
*/
	...
	//	logger.info("--------ReportDaoImpl getPLReport method start-------");

		List<ResignedResourcePLReport> resignedResourcePLReportList = new ArrayList<ResignedResourcePLReport>();
		List<BUResourcesPLReport> bUResourcesPLReportList = new ArrayList<BUResourcesPLReport>();
		List<ContractResourcePLReport> contractResourcePLReportList = new ArrayList<ContractResourcePLReport>();
		List<BorrowedResourcesPLReport> borrowedResourcesPLReportList = new ArrayList<BorrowedResourcesPLReport>();

		PLReport report = new PLReport();

		Query resignedResourceQuery;
		Query buResourcesQuery;
		Query contractResourceQuery;
		Query borrowedResourcesQuery;

		Session session = (Session) getEntityManager().getDelegate();

		Map<String, Object> mapPLReport = new HashMap<String, Object>();

		try {

			resignedResourceQuery = session
					.createSQLQuery(RmsNamedQuery.getResignedResourceQuery(includeResource))
					.setParameterList("buId", buId)
					.setParameter("startDate", startDate)
					.setParameter("endDate", endDate)
					.setParameterList("projectTypeIds", projectTypeIds);

			resignedResourcePLReportList = resignedResourceQuery.list();

			buResourcesQuery = session
					.createSQLQuery(RmsNamedQuery.getBUResourcesQuery(includeResource))
					.setParameterList("buId", buId)
					.setParameter("startDate", startDate)
					.setParameter("endDate", endDate)
					.setParameterList("projectTypeIds", projectTypeIds);
			...
			contractResourceQuery = session
					.createSQLQuery(RmsNamedQuery.getContractResourceQuery(includeResource))
					.setParameterList("buId", buId)
					.setParameter("startDate", startDate)
					.setParameter("endDate", endDate)
					.setParameterList("projectTypeIds", projectTypeIds);

			contractResourcePLReportList = contractResourceQuery.list();

			borrowedResourcesQuery = session
					.createSQLQuery(RmsNamedQuery.getBorrowedResources(includeResource))
					.setParameterList("buId", buId)
					.setParameter("startDate", startDate)
					.setParameter("endDate", endDate)
					.setParameterList("projectTypeIds", projectTypeIds);

			borrowedResourcesPLReportList = borrowedResourcesQuery.list();

			mapPLReport.put("resignedResourcePLReportList",
					resignedResourcePLReportList);
			mapPLReport.put("contractResourcePLReportList",
					contractResourcePLReportList);
			mapPLReport.put("bUResourcesPLReportList", bUResourcesPLReportList);

			mapPLReport.put("borrowedResourcesPLReportList",
					borrowedResourcesPLReportList);
...
/*		logger.info("--------ReportDaoImpl getPLReport method end-------");*/
	...
	@SuppressWarnings("unchecked")
	public List<BUResourcesPLReport> getMilindReport(Date startDate) {
		...
		try {
			...
			buResourcesQuery = session
					.createSQLQuery("SELECT r.yash_emp_id, CONCAT( TRIM(r.first_name),' ',TRIM(r.last_name)) AS NAME,"
			+"d.designation_name, b.name AS bu_name,"
			 ...
			 +"   g.grade, DATE((CASE WHEN ra.alloc_start_date < ('2016-01-01')"
			 +"         THEN ('2016-01-01')"
			 ...
			 +"     )"
			 +"   ) "
			 +"   -- AND ra.`curProj` IN (:projectTypeIds)" 
			 +" ORDER BY NAME ")
					.setParameter("startDate", startDate);
	...
	public Integer getReportCount() {
		...
		Query query = session
				.createSQLQuery("SELECT report_count FROM reports_details");
		Integer reportsCount = (Integer) query.uniqueResult();
		return reportsCount;
	}
	...
	public boolean updateJavaReportsCount(Integer countId) {
		...
		query = ((Session) this.entityManager.unwrap(Session.class))
				.createSQLQuery("UPDATE reports_details set report_count=:report_count");

		query.setParameter("report_count", countId);
		totalRowsDeleted = query.executeUpdate();
		return (totalRowsDeleted > 0) ? true : false;
