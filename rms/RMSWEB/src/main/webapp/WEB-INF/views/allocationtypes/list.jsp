	...
	$(document)
			.ready(
					function() {
						...
						$('#example')
								.delegate(
										...
										function(e) {
											...
											var text = "Are you sure you want to delete Allocation Type \""
													+ aData[1] + "\" ?";
											var successMessage = "Allocation Type \""
													+ aData[1]
													+ "\" has been successfully deleted.";
											if (id == undefined || id == null
													|| id == '') {
												text = "Are you sure you want to delete this row ?";
												successMessage = "Allocation Type has been successfully deleted.";
											}
								...
								.delegate(
										...
										function(e) {
											...
											if (nEditing !== null
												...
												//if(!validateInputs('.tbl') || !validateDuplicates('example', 'Allocation Type', 2)|| !duplicateValidation('example', 'Priority', 3)) {							
						...
						function duplicateValidation(tableId, screenId,
								columnId) {
							var flag = true;

							// Added for task # 252
							var number = $.trim(($('#' + tableId
									+ ' >tbody >tr').find("td:nth-child("
									+ columnId + ") input")).val());
							var inpSelect = number.replace(/^0+/, '');
							// Added for task # 252

							if (inpSelect == oldAllocationType) {
								flag = true;
							} else {
								<c:forEach var="allocation" varStatus="" items="${allocationtype}">
								var priority = "${allocation.priority}";
								if (priority.toUpperCase() == inpSelect
										.toUpperCase()) {
									flag = false;
								}
								</c:forEach>
						...
						function numericvalidation(tableId, screenId, columnId) {

							var inpSelect = $.trim(($('#' + tableId
									+ ' >tbody >tr').find("td:nth-child("
									+ columnId + ") input")).val());
							var priorityRegExp = /^[0-9]+$/;
							if (inpSelect != null || inpSelect != "") {

								if (!priorityRegExp.test(inpSelect)) {
									showError("Please enter only numeric values into Priority field");
								...
								// added for task # 253
									...
									showError("Priority field should be greater than zero");
									return false;
								}
							}
							return true;
						...
						function saveRow(oTable, nRow) {
							...
							/* if(spclCharacterValidation(stringName)){
								showError("Please Enter valid Allocation Type name");
								stopProgress();
								return false;
								...
								$
										.postJsonData(
												...
												function(data) {
													nEditing = null;
													stopProgress();
													var successMsg = "Allocation Type \""
															+ aData[1]
	...
	<div class="tab_section">
		...
		<div id='tab1' class="tab_div">
			<div class="search_filter_outer">
				<div class="search_filter allocationtypeAddnew"></div>
				...
				<table class="dataTbl display tablesorter addNewRow alignCenter"
					...
					<thead>
						<tr>
							...
							<th width="8%" align="center" valign="middle">Allocation
								Type</th>
							<th width="7%" align="center" valign="middle">Priority</th>
							<th width="4%" align="center" valign="middle">Edit</th>
							<th width="4%" align="center" valign="middle">Delete</th>
						</tr>
					</thead>
					<tbody>
						<c:forEach var="allocationtype" varStatus="alType"
							items="${allocationtype}">
							<tr id="${allocationtype.id}">
								<td align="center" valign="middle"><c:if
										test="${not empty allocationtype.id}">${alType.index+1}</c:if>
								</td>
								<td align="center" valign="middle"><c:if
										test="${empty allocationtype.allocationType}">N.A.</c:if> <c:if
										test="${not empty allocationtype.allocationType}">${allocationtype.allocationType}</c:if>
								</td>
								<td align="center" valign="middle"><c:if
										test="${empty allocationtype.priority}">N.A.</c:if> <c:if
										test="${not empty allocationtype.priority}">${allocationtype.priority}</c:if>
								</td>
