					...
					function() {
						...
						$('#example')
								.delegate(
										...
										function(e) {
											...
											var url = "currencys/" + id;
											var aData = oTable.fnGetData(nRow);
											var text = "Are you sure you want to delete Currency \""
													+ aData[1] + "\" ?";
											var successMessage = "Currency \""
													+ aData[1]
													+ "\" has been successfully deleted.";
											if (id == undefined || id == null
													|| id == '') {
												text = "Are you sure you want to delete this row ?";
												successMessage = "Currency has been successfully deleted.";
											}
						...
						function saveRow(oTable, nRow) {
							...
							if (spclCharacterValidation(stringName)) {
								showError("Please Enter valid currency name");
								stopProgress();
								return false;
				...
				<table class="dataTbl display tablesorter addNewRow alignCenter"
					...
					<thead>
						<tr>
							...
							<th width="8%" align="center" valign="middle">Currency Name</th>
							<th width="4%" align="center" valign="middle">Edit</th>
							<th width="4%" align="center" valign="middle">Delete</th>
						</tr>
					</thead>
					<tbody>
						<c:forEach var="currency" varStatus="curr" items="${currencys}">
							<tr id="${currency.id}">
								<td align="center" valign="middle"><c:if
										test="${not empty currency.id}">${curr.index+1}</c:if></td>
								<td align="center" valign="middle"><c:if
										test="${empty currency.currencyName}">N.A.</c:if> <c:if
										test="${not empty currency.currencyName}">${currency.currencyName}</c:if>
								</td>
