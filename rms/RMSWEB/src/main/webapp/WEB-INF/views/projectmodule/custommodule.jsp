...
<script type="text/javascript" charset="utf-8">
	...
	function valideDuplicates(tableId, screenId, columnId) {
		...
		if (inpSelect == oldActivity) {
			...
			var activity = "${activity.moduleName}";
						...
						function saveRow(oTable, nRow) {
							...
							if (aData[0] == null || $.trim(aData[0]) == '') {
								...
								jQuery
										.ajax({
											type : 'PUT',
											url : "/rms/modules/projects/update",
											data : jsonData,
											contentType : "application/json",
											success : function(data) {
												stopProgress();
				...
				<table class="dataTbl display tablesorter addNewRow alignCenter classer"  id="example">
					<thead>
						<tr>
							...
							<th width="8%" align="center" style="display:none;" valign="middle" id="name">Active</th>
					...
					<tbody>
                      <c:forEach var="activity" varStatus="act" items="${activitys}">
							<tr id="${activity.id}">
								...
								<td align="center" valign="middle"><c:if
										test="${empty activity.moduleName}">N.A.</c:if> <c:if
										test="${not empty activity.moduleName}">${activity.moduleName}</c:if>
								</td>
