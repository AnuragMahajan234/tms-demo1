...
$('.removePo').on("click", function(){	
		...
		$(document).ready(function(){
			...
			projectTable = $('#projectTableId').dataTable({
                "bProcessing": true,
                "bServerSide": true,       
                "sAjaxSource": '/rms/subModules/list/1',
                "sPaginationType": 'full_numbers',
                ...
                "fnServerData": function ( sSource, aoData, fnCallback ) {
				    ...
				    "fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
		                ...
		                if(managerReadonly==true){
		                	$('td:eq(0)', nRow).html('<a href="#" onclick="openMaintainance('+aData[1]+',\''+aData[2]+'\');">'+ aData[1]+'</a>');
		                }
		                else
		                {
		...
		$(".dd_project").change(function() {
    		...
    		//Added
    			...
    			//alert(r)
    			var Href = r+'?moduleId='+moduleId; //projectId
    			...
    			$("#customActivity").fancybox(
						{
							...
							afterClose: function(){
								/* $.getJSON("subModules/populateActiveInActiveSubModules/" + moduleId, function(data){
								...
								$.getJSON("subModules/populateActiveInActiveSubModules/" + moduleId, function(data){
								 ...
								 $.getJSON("subModules/populateActiveInActiveSubModules/" + moduleId, function(data){
									 var optionsHTML2 = '';
									 var optionsHTML3 = '';
									 $.each(data,function(ind2,elm2){
										 if(elm2.activeIndicator == "0"){
											optionsHTML3 += '<option value="'+elm2.subModuleId+'">'+elm2.subModuleName+'</option>';
									  		$('#pdo2').html(optionsHTML3);
									  	
										 }else{
											 optionsHTML2 += '<option value="'+elm2.subModuleId+'">'+elm2.subModuleName+'</option>';
											 $('#listMenu2').html(optionsHTML2);  
		...
		$('ul.tabs a[href="#tab1"]').click(function () {
			...
			$('a#ProjectActivityTabInactive').text("Project Sub Module");
		 ...
		 $('#projectActive').on('change', function() { 
			if(this.value=='0')
				...
				var oSettings = projectTable.fnSettings();
   				    oSettings.sAjaxSource  = "/rms/subModules/list/0";
   				...
   				{
   				var oSettings = projectTable.fnSettings();
   				    oSettings.sAjaxSource  = "/rms/subModules/list/1";
   				 	projectTable.fnClearTable();
   				 	projectTable.fnDraw();
   				}
...
function businessUnitValidate(businessUnit){
	...
	function optionGenerateHTML(obj) {
 		...
 		$.each(obj,function(ind,elm){
 			optionsHTML += '<option value="'+elm.id+'">'+elm.subModuleName+'</option>';
 		});
		return optionsHTML;
	}
		
	function openMaintainance(id,moduleName){
			startProgress();
			 
			displayMaintainance(moduleName);
			moduleId=id;
			 ...
			 $.getJSON("subModules/populateActiveInActiveSubModules/" + moduleId, function(data){
				 var optionsHTML = '';
				 var optionsHTML1 = '';
				 $.each(data,function(ind1,elm1){
					 if(elm1.activeIndicator == "0"){
						optionsHTML1 += '<option value="'+elm1.subModuleId+'">'+elm1.subModuleName+'</option>';
				  		$('#pdo2').html(optionsHTML1);
				  		
				  	//$.each(elm1,function(k5,v5) {
				  		//$('#pdo2').html(optionsHTML1 += '<option value="'+elm1.module.id+'"elm1'+elm1.subModuleName+'</option>');
				    //});
				  	
					 }else{
						 optionsHTML += '<option value="'+elm1.subModuleId+'">'+elm1.subModuleName+'</option>';
						 $('#listMenu2').html(optionsHTML);  
						 
					//	$.each(elm1,function(k5,v5) {
						//	$('#listMenu2').html(optionsHTML += '<option value="'+elm1.module.id+'">'+elm1.subModuleName+'</option>');
					//	});
		...
		function displayMaintainance(moduleName){
			 
			$('a#ProjectActivityTabInactive').text(moduleName);
...
function moveDown(lst){
	if (lst.selectedIndex<lst.options.length-1){
		...
		{ 
		 ...
		 $.ajax({
			        url: 'subModules/changeProjectModuleStatus',
			      	contentType: "application/json",
			      	type:"GET",
			     	data: "moduleId=" + moduleId +"&projectModuleMapped="+projectModuleMapped+"&projectModuleNotMapped="+projectModuleNotMapped,
			     	success: function(succeResponse) { 
			     		showSuccess("Project Sub Modules has been saved successfully ");
			     		$("#tab1").show();
			     		$("#tab2").hide();
			     		$('#ProjectActivityTabInactive').off('click');
						$('#ProjectActivityTabInactive').removeClass('active').addClass('MaintenanceTab');
			 	    ...
			 	    {
			 	    	showError("Project Sub Modules cannot be  updated.");
			 	    }
			 });  
	...
	<div class="tab_seaction">
		<ul class='tabs'>
			...
			<li><a class="ProjectActivityTab MaintenanceTab"
				id="ProjectActivityTabInactive" href='#tab2'>Project Sub Module
			</a></li>
		</ul>
		<div id='tab1' class="tab_div">
			<div class="tbl">
				<table class="dataTbl display tablesorter dataTable"
					...
					<thead>
						<tr>
							<th align="center" valign="middle">Module ID</th>
							<th align="center" valign="middle">Module ID</th>
							<th align="center" valign="middle">Module Name</th>

							<!-- <th align="center" valign="middle">Sub Module Name</th> -->
							<th align="center" valign="middle">Project Name</th>
							<th align="center" valign="middle">Project BG BU</th>
							<th align="center" valign="middle">Customer Name</th>

						</tr>
						<tr class="">
							<td><input type="text" name="search_moduleId"
								value="Module ID" class="search_init" hidden="true"/></td>
							<td><input type="text" name="search_moduleId"
								value="Module ID" class="search_init" disabled="disabled"/></td>
							<td><input type="text" name="search_moduleName"
								value="Module Name" class="search_init" /></td>
							<td><input type="text" name="search_projectName"
								value="Project Name" class="search_init" /></td>
							<td><input type="text" name="search_BuCode"
								value="BG BU Name" class="search_init" /></td>
		...
		<div id='tab2' class="tab_div" style="display: none;">
						...
						<table width="100%" border="0" cellpadding="2" cellspacing="1"
							...
							<tr>
								<td width="58%" valign="top"><table border="0" align="left"
										...
										<th align="left">Map Sub Module</th>
			...
			<br> <a href="#" data-href="subModules/customProjectSubmodules"
				id="customActivity" class="various"> Add Project Sub Module</a> <br>
			<br>
...
<div class="notification-bar">
	...
	<sec:authorize access="hasAnyRole('ROLE_ADMIN')">
		<div class="notification-text">You can add/map sub modules for
			any projects by clicking on its project id.</div>
	</sec:authorize>
	<sec:authorize
		access="hasAnyRole('ROLE_MANAGER','ROLE_DEL_MANAGER', 'ROLE_BG_ADMIN')">
		<div class="notification-text">You can add/map sub modules for
			your projects by clicking on its project id.</div>
	</sec:authorize>
</div>
