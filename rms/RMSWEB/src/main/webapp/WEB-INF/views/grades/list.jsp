					...
					function() {
						...
						$('#example')
								.delegate(
										...
										function(e) {
											...
											var text = "Are you sure you want to delete Grade \""
													+ aData[1] + "\" ?";
											var successMessage = "Grade \""
													+ aData[1]
													+ "\" has been successfully deleted.";
											if (id == undefined || id == null
													|| id == '') {
												text = "Are you sure you want to delete this row ?";
												successMessage = "Grade has been successfully deleted.";
											}
						...
						function saveRow(oTable, nRow) {
							...
							if (spclCharacterValidation(stringName)) {
								showError("Please Enter valid grade name");
								stopProgress();
								return false;
				...
				<table class="dataTbl display tablesorter addNewRow alignCenter"
					...
					<thead>
						<tr>
							...
							<th width="8%" align="center" valign="middle">Grade</th>
							<th width="4%" align="center" valign="middle">Edit</th>
							<th width="4%" align="center" valign="middle">Delete</th>
						</tr>
					</thead>
					<tbody>
						<c:forEach var="grade" varStatus="gradeState" items="${grade}">
							<tr id="${grade.id}">
								<td align="center" valign="middle"><c:if
										test="${not empty grade.id}">${gradeState.index+1}</c:if></td>
								<td align="center" valign="middle"><c:if
										test="${empty grade.grade}">N.A.</c:if> <c:if
										test="${not empty grade.grade}">${grade.grade}</c:if></td>
