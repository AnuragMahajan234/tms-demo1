			...
			.ready(
					function() {
											...
											jQuery
													.ajax({
														...
														success : function(data) {
															showSuccess("Custom Activity has been Successfully deleted");
															window.location
																	.reload();
						...
						function saveRow(oTable, nRow) {
							...
							jsonData += '}';
							//alert("json gggggg"+jsonData);
							if (aData[0] == null || $.trim(aData[0]) == '') {

								jQuery
										.ajax({
											type : 'POST',
											url : "/rms/projectactivitys",
											data : jsonData,

											contentType : "application/json",
											success : function(data) {
												stopProgress();
												var successMsg = "Custom Activity \""
														+ aData[1]
					...
					<tbody>
						<c:forEach var="activity" varStatus="act" items="${activitys}">
							<tr id="${activity.id}">
								...
								<td align="center" valign="middle"><c:set var="flag"
										value="false" /> <c:forEach var="custom" items="${customIDs}">
										<c:if test="${custom == activity.id}">
