...
$('.removePo').on("click", function(){	
    		...
    		//Added
    			/* $(".various").fancybox({
    				onClose:function () {
    					...
    					var selectDropdownDefault =  document.getElementById("pdo");
    					var selectDropdownDefaultTo =  document.getElementById("listMenu");
    					var skillIdOptionDefault=[];
    					var skillOptionDefault=[];
    					
    					 <c:forEach var="defaultActivityList" items="${activitys}">
    			...
    			$("#customActivity").fancybox(
						{
							...
							afterClose: function(){
								/* $.getJSON("projectactivitys/populateActivity/" + projectId, function(data){
									$('#pdo2').html(optionGenerateHTML(data));
								  });
								$.getJSON("projectactivitys/selectedCustomActivity/" + projectId, function(data){
										$('#listMenu2').html(optionGenerateHTML(data));
								 });
								 */
								$.getJSON("projectTickets/ticketPriorities/" + projectId, function(data){
							 		$('#pdo').html(optionGenerateHTML(data));
							 	});
							 	
							 	$.getJSON("projectTickets/selectedTicketPriorities/" + projectId, function(data){
							 		$('#listMenu').html(optionGenerateHTML(data));
							 	});
							  	$("#tab2").load();
    			...
    			$("#customActivity1").on("click",function(){
        			var r =$(this).attr("data-href");//this.href;
        			var Href = r+'?projectId='+projectId;
        	
        			$("#customActivity1").fancybox(
    						{
    							autoSize : false,
    							closeClick : true,
    							autoDimensions : true,
    							...
    							afterClose: function(){
    								...
    								$.getJSON("projectStatus/ticketStatus/" + projectId, function(data){
    									$('#pdo1').html(optionGenerateHTML1(data));
    								});
    								 	
    								$.getJSON("projectStatus/selectedTicketStatus/" + projectId, function(data){
    									$('#listMenu1').html(optionGenerateHTML1(data));
    								});
		...
		$('ul.tabs a[href="#tab1"]').click(function () {
			...
			$('a#ProjectActivityTabInactive').text("Project Ticket");
...
function businessUnitValidate(businessUnit){
	...
	function optionGenerateHTML(obj) {
		...
		$.each(obj,function(ind,elm){
 			optionsHTML += '<option value="'+elm.id+'">'+elm.priority+'</option>';
 		});
		return optionsHTML;
	}
	
	function optionGenerateHTML1(obj) {
 		var optionsHTML = '';
		$.each(obj,function(ind,elm){
 			optionsHTML += '<option value="'+elm.id+'">'+elm.status+'</option>';
 		});
		return optionsHTML;
	}
	...
	function openMaintainance(id,projectName){
			...
			displayMaintainance(projectName);
			 	...
			 	$('#pdo').empty(); $('#listMenu').empty();
			 	$('#pdo1').empty(); $('#listMenu1').empty();
			 	
			 	/*$('#pdo2').empty(); $('#listMenu2').empty(); */
			 	$("#customTable").show();
			 	
			 	/* $.getJSON("projectactivitys/defaultActivity/" + id, function(data){
			 		$('#pdo').html(optionGenerateHTML(data));
			 	}); */
			 	
			 	$.getJSON("projectTickets/ticketPriorities/" + id, function(data){
			 		$('#pdo').html(optionGenerateHTML(data));
			 	});
			 	
			 	$.getJSON("projectTickets/selectedTicketPriorities/" + id, function(data){
			 		$('#listMenu').html(optionGenerateHTML(data));
			 	});
			 	
			 	$.getJSON("projectStatus/ticketStatus/" + id, function(data){
			 		$('#pdo1').html(optionGenerateHTML1(data));
			 	});
			 	
			 	$.getJSON("projectStatus/selectedTicketStatus/" + id, function(data){
			 		$('#listMenu1').html(optionGenerateHTML1(data));
			 	});
			 	
			 	/* sarang added */
			   
			  /* delete this block $.getJSON("projectactivitys/sepgActivity/" + id, function(data){
			 		$('#pdo1').html(optionGenerateHTML(data));
			  });
			  ...
			  $.getJSON("projectactivitys/selectedDefaultActivity/" + projectId, function(data){
			 		$('#listMenu').html(optionGenerateHTML(data));
			  });
			 
			  $.getJSON("projectactivitys/selectedSEPGActivity/" + projectId, function(data){
			 		$('#listMenu1').html(optionGenerateHTML(data));
			  });
...
function moveDown(lst){
	if (lst.selectedIndex<lst.options.length-1){
		...
		function saveactivity() { 
		    ...
		    var projectStatus = [];
			var responseMsg = "Project ";
			var saved = false;
			
		    $('#listMenu option').each(function(i) { 
		    	projectPriorities.push( $(this).attr('value') );
				 
			});
		    $('#listMenu1 option').each(function(i) { 
		    	projectStatus.push( $(this).attr('value') );
				 
			});
				
			/* if(projectPriorities.length == 0){
				projectPriorities.push("0");
			}
			if(projectStatus.length == 0){
				projectStatus.push("0");
			} */
			
	
			/* if(projectPriorities==0){
				showError("Please Select atleast one Project Priority");
			}else{ */
		 		$.ajax({
			        url: 'projectTickets/saveProjectPriorities',
			        async: false,
			      	contentType: "application/json",
			      	type:"GET",
			     	data: "projectPrioritiesList=" +projectPriorities+"&projectId="+projectId,
			     	success: function(succeResponse) {
			     		/* showSuccess(" Project Priorities has been saved successfully "); */
			     		saved = true;
			     		$("#tab1").show();
			     		$("#tab2").hide();
			     		$('#ProjectActivityTabInactive').off('click');
			     	...
			     	error: function(errorResponse) {
			     		saved = false;
			 	    	showError("Project Priorities cannot be  updated.");
			 	    }
				});  
			/* } */

			/* if(projectStatus==0){
				showError("Please Select atleast one Project Status");
			}else{ */
		 		$.ajax({
			        ...
			        async: false,
			      	contentType: "application/json",
			      	type:"GET",
			     	data: "projectStatusList=" +projectStatus+"&projectId="+projectId,
			     	success: function(succeResponse) { 
			     		/* alert(succeResponse);
			     		showSuccess(" Project Status has been saved successfully "); */
			     		saved = true;
			     		$("#tab1").show();
			     		$("#tab2").hide();
			     		$('#ProjectActivityTabInactive').off('click');
			     	...
			     	error: function(errorResponse) {
			     		saved = false;
			 	    	showError("Project Status cannot be  updated.");
			 	    }
				});  
			/* } */
				if(saved) {
					showSuccess("Project Ticket Priorities/Status has been saved successfully");	
				} else {
					showError("Project Ticket Priorities/Status cannot be updated.");
				}
	...
	<div class="tab_seaction">
		<ul class='tabs'>
			...
			<li><a class="ProjectActivityTab MaintenanceTab"
				id="ProjectActivityTabInactive" href='#tab2'>Project Ticket </a></li>
		</ul>
			...
			<table width="100%" border="0" cellspacing="1" cellpadding="0">
				<tr>
					<td><form name="form1" method="post" action="">
												...
												<td width="58%" valign="top">
													<div class="defaultActivityWrapper">
														<table border="0" align="left" cellpadding="2"
															...
															<th align="left">Map Priority</th>
															<tr></tr>
															<tr align="center" valign="middle">
																<td align="left" valign="top"
																	class="ent_elevenred defaultActivityHeading">
																	Priority</td>
																...
																<td class="text"><table width="60%" border="0"
																		...
																		<tr>
																			<td align="center" valign="middle"><input
																				name="btn_Exit" type="button" class="ent_button"
																				id="btn_Exit" value="&lt;&lt;"
																				onClick="move_item(listMenu,pdo)"></td>
																		</tr>
																	</table></td>
												...
												<td width="58%" valign="top">
													<div class="defaultActivityWrapper">
														<table border="0" align="left" cellpadding="2"
															...
															<tr align="center" valign="middle">
																<td valign="top" class="text"><span
																	id="selectionView"> <select name=pdo1 size="7"
							...
							<br> <a href="#"
								data-href="projectTickets/customTicketPriority" 
								id="customActivity" class="various"> Add Ticket Priority</a>
							&nbsp; &nbsp;
							<a href="#"
								data-href="projectStatus/customTicketStatus" 
								id="customActivity1" class="various"> Add Ticket Status</a>	
							<br>
							<br>
...
<div class="notification-bar">
	...
	<sec:authorize
		...
		<div class="notification-text">Click on "Project Id" to add/map
			Ticket Priority or Status for your projects.</div>


	</sec:authorize>
	<sec:authorize access="hasAnyRole('ROLE_ADMIN' )">
		<div class="notification-text">Click on "Project Id" to add/map
			Ticket Priority or Status for any projects.</div>
	</sec:authorize>
</div>
