...
function valideDuplicates(tableId, screenId, columnId) {
	...
	if (inpSelect == oldActivity) {
		...
		var activity = "${activity.priority}";
			...
			function cancelDataRow(id, empId, projId) {
				...
				$('#example a.delete').live('click',
					function(e) {
						...
						if (id == undefined || id == null || id == '') {
							var msg = "Project Ticket Priority has been Successfully deleted";
						       cancelDataRow(null, null, null);
						...
						jQuery.ajax({
							...
							success : function(data) {
								  ...
								  stopProgress();
							      var successMsg1 = "Project Priority  have been deleted successfully";
				...
				$('#example a.edit').live('click',
					function(e) {
						...
						// Provide the required parameters in the below functions to validate the inputs
							if (!validateInputs('.tbl') || !valideDuplicates('example', 'Priority', 2)) {
								return;
							}
						...
						function saveRow(oTable, nRow) {
							...
							if (stringName == "") {
								showError("Please Enter Priority name");
								stopProgress();
								return false;
							} else if (spclCharacterValidation(stringName)) {
								showError("Please Enter valid Priority name");
								stopProgress();
								return false;
 ...
 */
							...
							if (aData[0] == null || $.trim(aData[0]) == '') {
								jQuery.ajax({
									type : 'POST',
									url : "/rms/projectTickets",
									data : jsonData,
									contentType : "application/json",
									success : function(data) {
										stopProgress();
										var successMsg = "Project Priority \""+ aData[1] + "\" have been saved successfully";
											showSuccess(successMsg);
											reloadFunction();
									},
									error : function(data) {
										stopProgress();
										showError("Priority"+ " \""+ aData[1]+ "\" is already present for this project. Please provide different name");
									}
								});
							} else {
								jQuery.ajax({
									...
									contentType : "application/json",
										success : function(data) {
											...
											error : function(data) {
												stopProgress();
												showError("Project Priority}"+ " \""+ aData[1] + "\" is already present. Please provide different name");
											}
								});
							}
							}
				...
				<table class="dataTbl display tablesorter addNewRow alignCenter" id="example">
					<thead>
						<tr>
							...
							<th width="8%" align="center" valign="middle" id="name">Priority</th>
					...
					<tbody>
						<c:forEach var="priority" varStatus="act" items="${activitys}">
							<tr id="${priority.id}">
								<td align="center" valign="middle"><c:if test="${not empty priority.id}">${act.index+1}</c:if></td>
								<td align="center" valign="middle">
									<c:if test="${empty priority.priority}">N.A.</c:if> 
									<c:if test="${not empty priority.priority}">${priority.priority}</c:if>
								</td>
								<%-- <td align="center" valign="middle">
									<c:if test="${empty priority.mandatory}">
										<select name="mandatory">
											<option value="yes">Yes</option>
											<option value="no">No</option>
										</select>
									</c:if>
									<c:if test="${priority.mandatory=='false'}">No</c:if>
									<c:if test="${priority.mandatory=='true'}">Yes</c:if>
								</td> --%>
