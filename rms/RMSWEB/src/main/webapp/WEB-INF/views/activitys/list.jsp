			...
			.ready(
					function() {
						...
						$('#addNew')
								.click(
										function(e) {
											...
											$("#example tbody tr:first").find(
													"td").find("a.cancel")
													.addClass("delAddedRow");
						...
						function saveRow(oTable, nRow) {
	                        ...
	                        var numbers = /^[0-9]+$/;
							if (!aData[3].match(numbers)) {
								var errorMsg = "Please enter MAX value as numeric !";
								showError(errorMsg);
								stopProgress();
							}
							else if(aData[3].match(numbers)){
							var jsonData = '{'
							...
							if (aData[0] == null || $.trim(aData[0]) == '') {
								$
										.postJsonData(
												...
												function(data) {
													...
													setTimeout(function() {
														location.reload(); //edit by meet&akash	
													}, 1000);
												}, 'json');
								...
								$
										.putJsonData(
												...
												jsonData,
												function(data) {
													//event.preventDefault();      //commint this method for mozilla browser  
													nEditing = null;
													stopProgress();
		...
		<div id='tab1' class="tab_div">
			...
			<div class="tbl">
				<table class="dataTbl display tablesorter addNewRow alignCenter"
					id="example" style="margin-top:1px;">
					<thead>
						<tr>
							<th width="2%" align="center" valign="middle">S. No.</th>
							<th width="8%" align="center" valign="middle" id="name">Activity
								Name</th>
							<th width="3%" align="center" valign="middle">Project Mandatory</th>
							<th width="2%" align="center" valign="middle">Type</th>
							<th width="2%" align="center" valign="middle">Max</th>
							<th width="3%" align="center" valign="middle">Format</th>
					...
					<tbody>
						<c:forEach var="activity" varStatus="act" items="${activitys}">
							<tr id="${activity.id}">
								...
								<td align="center" valign="middle"><c:if
										test="${empty activity.type}">N.A.</c:if> <c:if
										test="${not empty activity.type}">${activity.type}</c:if></td>
								<td align="center" valign="middle"><c:if
										test="${empty activity.max}">N.A.</c:if> <c:if
										test="${not empty activity.max}">${activity.max}</c:if></td>
								<td align="center" valign="middle"><c:if
										test="${empty activity.format}">N.A.</c:if> <c:if
										test="${not empty activity.format}">${activity.format}</c:if>
								</td>
								<td align="center" valign="middle"><c:if
