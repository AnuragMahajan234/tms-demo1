					...
					function() {
						...
						$('#example')
								.delegate(
										...
										function(e) {
											...
											var url = "invoicebys/" + id;
											var aData = oTable.fnGetData(nRow);
											var text = "Are you sure you want to delete Invoice By \""
													+ aData[1] + "\" ?";
											var successMessage = "Invoice By \""
													+ aData[1]
													+ "\" has been successfully deleted.";
											if (id == undefined || id == null
													|| id == '') {
												text = "Are you sure you want to delete this row ?";
												successMessage = "Invoice By has been successfully deleted.";
											}
						...
						function saveRow(oTable, nRow) {
							...
							if (spclCharacterValidation(stringName)) {
								showError("Please Enter valid invoice by name !<br> (first letter must be a character)");
								stopProgress();
								return false;
				...
				<table class="dataTbl display tablesorter addNewRow alignCenter"
					...
					<thead>
						<tr>
							...
							<th width="8%" align="center" valign="middle">Invoice By
								Name</th>
					...
					<tbody>
						<c:forEach var="invoiceBy" varStatus="inv" items="${invoicebys}">
							<tr id="${invoiceBy.id}">
								<td align="center" valign="middle"><c:if
										test="${not empty invoiceBy.id}">${inv.index+1}</c:if></td>
								<td align="center" valign="middle"><c:if
										test="${empty invoiceBy.name}">N.A.</c:if> <c:if
										test="${not empty invoiceBy.name}">${invoiceBy.name}</c:if></td>
