						...
						$('#example')
								.delegate(
										...
										function(e) {
											...
											var text = "Are you sure you want to delete Module \""
													+ aData[1] + "\" ?";
											var successMessage = "Module \""
													+ aData[1]
								...
								.delegate(
										...
										function(e) {
											...
											if (nEditing !== null
												...
												// Pass the table class as parameter in validateInputs function
						...
						function saveRow(oTable, nRow) {
							...
							if (aData[0] == null || $.trim(aData[0]) == '') {
								$
								 ...
								 $
										.postJsonData(  //Module/Ticket update issue: Added by Pratyoosh
												"modules",
												jsonData,
												function(data) {
													saveOpen = false;
				...
				<table class="dataTbl display tablesorter addNewRow alignCenter"
					...
					<thead>
						<tr>
							...
							<th width="8%" align="center" valign="middle">Module /
								Ticket No. Name</th>
					...
					<tbody>
						<c:forEach var="module" varStatus="moduleState" items="${modules}">
							<tr id="${module.id}">
								<td align="center" valign="middle"><c:if
										test="${not empty module.id}">${moduleState.index+1}</c:if></td>
								<td align="center" valign="middle"><c:if
										test="${empty module.moduleName}">N.A.</c:if> <c:if
										test="${not empty module.moduleName}">${module.moduleName}</c:if>
								</td>
