...
// 				var saveOpen = false;
				...
				$('#example a.delete').live('click', function (e) {
					...
					var text = "Are you sure you want to delete Rate Id \"" + aData[1] +"\" ?";
					var successMessage = "Rate Id \"" + aData[1] +"\" has been successfully deleted.";
					if (id == undefined || id == null || id == '') {
	 	            	text = "Are you sure you want to delete this row ?";
	 	            	successMessage = "Rate Id has been successfully deleted.";
	 	            }
				...
				$('#example a.edit').live('click', function (e) {
					...
					if ( nEditing !== null && nEditing != nRow ) {
						...
						if(!validateInputs('.tbl') || !validateDuplicates('example', 'Rate Id', 2)){
							return;
						}
	...
	function saveRow (oTable, nRow) {
		...
		sData.push({name:"billingRate",value:aData[1]});
		
		var stringName = aData[1] ;
		if(spclCharacterValidation(stringName)){
			showError("Please Enter valid billing rate");
			 stopProgress();
			return false;
		...
		if(aData[0] == null || $.trim(aData[0]) == ''){
			$.postJsonData("rateids", jsonData, function(data) {
				saveOpen = false;
				nEditing= null;
				stopProgress();
				var successMsg ="Rate Id \"" + aData[1] +"\" has been inserted successfully";
				showSuccess(successMsg);
				setTimeout(function() {
					location.reload();	
				}, 1000);		 },  "json" );
		} else {
			$.putJsonData("rateids", jsonData, function(data) {
				saveOpen = false;
				nEditing= null;
				stopProgress();
				var successMsg ="Rate Id \"" + aData[1] +"\" details have been saved successfully";
				showSuccess(successMsg);
				setTimeout(function() {
					location.reload();	
				}, 1000);
                ...
                <table  class="dataTbl display tablesorter addNewRow alignCenter" id="example" >
                  <thead>
                    <tr>
                      ...
                      <th width="8%" align="center" valign="middle">Rate Name</th>
                      <th width="4%" align="center" valign="middle">Edit</th>
						<th width="4%" align="center" valign="middle">Delete</th>                     
                    </tr>
                  </thead>
                  <tbody>                  
                  <c:forEach var="billingScale" varStatus="rate" items="${billingScales}">
			         <tr id="${billingScale.id}">
			                <td align="center" valign="middle">
			                <c:if test="${not empty billingScale.id}">${rate.index+1}</c:if>
			                </td>
			                <td align="center" valign="middle">
			                <c:if	test="${empty billingScale.billingRate}">N.A.</c:if> <c:if
									test="${not empty billingScale.billingRate}">${billingScale.billingRate}</c:if>
			                </td>
			                <td align="center" valign="middle">
