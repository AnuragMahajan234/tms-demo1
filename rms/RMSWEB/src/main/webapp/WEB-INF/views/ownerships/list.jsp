					...
					function() {
						...
						$('#example')
								.delegate(
										...
										function(e) {
											...
											var url = "ownerships/" + id;
											var aData = oTable.fnGetData(nRow);
											var text = "Are you sure you want to delete Ownership \""
													+ aData[1] + "\" ?";
											var successMessage = "Ownership \""
													+ aData[1]
													+ "\" has been successfully deleted.";
											if (id == undefined || id == null
													|| id == '') {
												text = "Are you sure you want to delete this row ?";
												successMessage = "Ownership has been successfully deleted.";
											}
						...
						function saveRow(oTable, nRow) {
							...
							if (spclCharacterValidation(stringName)) {
								showError("Please Enter valid ownership name");
								stopProgress();
								return false;
				...
				<table class="dataTbl display tablesorter addNewRow alignCenter"
					...
					<thead>
						<tr>
							...
							<th width="8%" align="center" valign="middle">Ownership Name</th>
							<th width="4%" align="center" valign="middle">Edit</th>
							<th width="4%" align="center" valign="middle">Delete</th>
						</tr>
					</thead>
					<tbody>
						<c:forEach var="ownership" varStatus="owner" items="${ownerships}">
							<tr id="${ownership.id}">
								<td align="center" valign="middle"><c:if
										test="${not empty ownership.id}">${owner.index+1}</c:if></td>
								<td align="center" valign="middle"><c:if
										test="${empty ownership.ownershipName}">N.A.</c:if> <c:if
										test="${not empty ownership.ownershipName}">${ownership.ownershipName}</c:if>
								</td>
