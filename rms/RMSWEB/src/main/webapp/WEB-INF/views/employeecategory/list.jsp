					...
					function() {
						...
						$('#example')
								.delegate(
										...
										function(e) {
											...
											var url = "employeeCategories/"
													+ id;
											var aData = oTable.fnGetData(nRow);
											var text = "Are you sure you want to delete EmployeeCategory \""
													+ aData[1] + "\" ?";
											var successMessage = "EmployeeCategory \""
													+ aData[1]
													+ "\" has been successfully deleted.";
											if (id == undefined || id == null
													|| id == '') {
												text = "Are you sure you want to delete this row ?";
												successMessage = "EmployeeCategory has been successfully deleted.";
											}
						...
						function saveRow(oTable, nRow) {
							...
							if (spclCharacterValidation(stringName)) {
								showError("Please Enter valid EmployeeCategory name");
								stopProgress();
								return false;
								...
								$
										.postJsonData(
												...
												function(data) {
													...
													nEditing = null;
													stopProgress();
													var successMsg = "EmployeeCategory \""
															+ aData[1]
				...
				<table class="dataTbl display tablesorter addNewRow alignCenter"
					...
					<thead>
						<tr>
							...
							<th width="8%" align="center" valign="middle">Employee
								Category Name</th>
					...
					<tbody>
						<c:forEach var="employeeCategory" varStatus="category"
							items="${employeecategory}">
							<tr id="${employeeCategory.id}">
								<td align="center" valign="middle"><c:if
										test="${not empty employeeCategory.id}">${category.index+1}</c:if>
								</td>
								<td align="center" valign="middle">
									<%--   <c:if test="${empty employeeCategory.id}">N.A.</c:if> <c:if
								  test="${not empty employeeCategory.id}"> --%>
									${employeeCategory.employeecategoryName} <%-- 								  </c:if> --%>
								</td>
