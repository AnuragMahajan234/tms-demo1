...
<script>
					...
					function() {
						...
						$('#projectId').change(
								function() {
									...
									if (serverCallRequired && oValue
											&& $('#locationId').empty()) {

										$.getJSON(
												"/rms/UtilizationReport/getLocation/"
														+ oValue, {},
												showLocation);
								...
								.multiselect(
										{
											...
											close : function(event) {
												var oValue = $('#orgHierarchy')
														.val();

												if (serverCallRequired
														&& (oValue != null || oValue != -1)) {
													...
													var aUrl = "/rms/UtilizationReport/getProject/"
															+ oValue;
													$.getJSON(
															"/rms/UtilizationReport/getLocation/"
																	+ oValue,
						...
						function showLocation(data) {
							...
							if (data.length != 0) {
								...
								$('#locationId').multiselect('refresh')
										.multiselect();
								...
								$
										.each(
												...
												function(key, val) {
													...
													htmlVar += '<option value="'+data[key].employeeId+'">'
															+ data[key].firstName
															+ data[key].lastName
															+ "["
															+ data[key].yashEmpId
						...
						$('#locationId').change(
								function() {
									...
									if (serverCallRequired && bids && pids
											&& lids && $('#resource').empty()) {

										$.getJSON(
												"/rms/UtilizationReport/getResource/"
														+ bids + '/' + pids
														+ '/' + lids, {},
												showResource);
	...
	function exportDataToExcel() {
		...
		if (document.getElementById("startDatepicker").value == "") {
			...
			validationFlag = false;
				...
				&& document.getElementById("endDatepicker").value < document
		...
		if (!validationFlag) {
			...
			if (document.getElementById("endDatepicker").value != "") {
				requestParam = requestParam + "endDate="
			...
			var selectedCount = parseInt($(
					$('select#resource').next('button').find('span')[1]).text()
					.split(' ')[0]);
			if ($('select#resource').find('option').length === selectedCount
					|| selectedCount > 100) {
				employeeList = [ "0" ];
			}
			...
			var _href = "getCustUtilizationDetailReport"; //$('#exportToExcel').attr('href');
	...
	function submitDetail() {
		...
		if (!validationFlag) {
			...
			var requestParam = "orgIdList=" + orgIdList + "&locIdList="
					+ locIdList + "&projIdList=" + projIdList
					+ "&customerList=" + customerList + "&date=" + date
					+ "&employeeList=" + employeeList;
			...
			$
					.ajax({
						...
						url : '/rms/UtilizationReport/getCustUtilizationDetailReport',
						contentType : "application/json; charset=utf-8",
						async : false,
						data : requestParam,
						dataType : 'json',
						...
						error : function(errorResponse) {
							...
							console
									.log("Error result: Error trying to get data for resource movement report ");
...
<body>
	<div class="content-wrapper">
		<div class="botMargin">
			<h1>Customized Utilization Detail Report</h1>
		</div>
		<div class="tab_seaction">
			...
			<div id='tab1' class="tab_div">
				<div class="form">
					<form id="RMReport" name="RMReport"
						action="UtilizationReportController">

						<table id="RMReportTable" cellspacing="10" cellpadding="10">
							<tr>

								<td align="left">Start Date :</td>
								<td align="left"><input type="text" id="startDatepicker"
							...
							<tr>
								...
								<td>
									<div class="positionRel">
										...
										<select name="projects.id" id="projectId" multiple="multiple"
											class="dropdown-search" onchange="onProjectChange()">
								...
								<td>
									<div class="positionRel">
										<select name="locations.id" id="locationId"
											multiple="multiple" class="dropdown-search"
											onchange="onLocationChangeFunction()">
								...
								<td>Employee Name :</td>
								<td><div class="positionRel">
										<select name="resources.employeeId" id="resource"
											multiple="multiple" class="dropdown-search">
