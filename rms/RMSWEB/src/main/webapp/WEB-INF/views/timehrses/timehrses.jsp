...
<td align="left" id="approveStatusId{{:#index}}" >
			...
			{{if approveStatus == 'A' }}
				...
				<sec:authorize access="hasAnyRole('ROLE_ADMIN','ROLE_SEPG_USER','ROLE_DEL_MANAGER','ROLE_BG_ADMIN','ROLE_BEHALF_MANAGER','ROLE_MANAGER')">
					<a href="#comment" id="comment_timehrsList" title="Change To Review Status"><img src="resources/images/approve_icon.png" onclick="rejectTimesheet({{>id}},'{{>approveStatus}}'); "height="16" width="16" /></a>
				</sec:authorize>
	...
	//Timesheet Approval Validation Start:Pratyoosh
	    ...
	    if(document.getElementById("billedHrs"+id)!=null){
		  ...
		  if(!planRegExp.test(plannedHours) && plannedHours != 0){		
			  errorMsg = errorMsg + ("\u2022 Zero and alphanumeric characters is not allowed in Plannned Hours! <br/>");
			 } 
 		
	    
	     	if(!billedRegExp.test(billedHours) && billedHours != 0) {						
	    	 	errorMsg = errorMsg + ("\u2022  Zero and alphanumeric characters is not allowed in Billed Hours! <br/>");
		 } 
	 	
		if ((plannedHours == '')  || (billedHours == '')  ){
   		 	errorMsg = ("\u2022 Planned and Billed Hours should not be empty OR ZERO! <br/>");
   		}
...
//Timesheet Approval Validation End:Pratyoosh
	...
	var jsonData ={"id":resourceAllocId,"employeeId":{employeeId : resourceEmployeeId}};
	if(document.getElementById("plannedHrs"+id)!=null)
	  plannedhrs = document.getElementById("plannedHrs"+id).value;
	if(approve) {
		startProgress();	
		$.postJson('timehrses/'+id+'/A'+'/true/'+billedhrs+'/'+plannedhrs+'/'+remarks+'/'+resourceEmployeeId+'/'+resourceAllocId, jsonData, function(data){	
			var id = document.getElementById("employeeIdHidden").value;
			getTimehrsByProjectId();
			saveOpen = false;
			nEditing= null;   
...
var gEmployeeName;
var gResourceId;
var projectAllocNavigationFlag;
       // Wait until the DOM has loaded before querying the document
		$(document).ready(function(){
		$("#comment").hide();
		navigationFlag="${navigationFlag}"; 
		projectAllocNavigationFlag="${projectAllocNavigationFlag}";
		$("#NoAllocMessage").hide();
		$(".tab_div").hide();
...
//   			$("#updateRecords").reset();
  		...
  		$('#ok').on("click", function(){
  			...
  			if(rejectionRemarks.length  == "" || rejectionRemarks == null ){
  				...
  				var successMsg ="TimeSheet has been rejected successfully "; 
  					...
  					setTimeout(function(){
  						$("#ToolTables_timehrsTableId2_0").hide();	
  					},1000)
  		...
  		var cells = [];
  		 $('#approveAll').on("click", function(){ 
  		    ...
  		    var latestWeekEndDate=$("#datepickerchange").val();
  		  	
  		    var projectAllocStatus="${projectAllocStatus}";
  				...
  				if(!reviewDone)
  							...
  							return;
  								...
  								$.ajax({  
  							        url: "",  
  							        dataType: 'json',  
  							        data: {find:"approveAll" , resourceId:id,plannedHours: plannedHours ,billedHours: billedHours , weekendDate: weekendDate ,resourceAllocId: resourceAllocId,remarks:remarks,projectId:projectId, latestWeekEndDate:latestWeekEndDate,allocationStatus:projectAllocStatus},  
  							        async: false,  
  							        success: function(){  
  	...
  	$(document.body).on('change','#activeOrAll',function(){
		    ...
		    var newTimeSheetStatus=document.getElementById('newTimeSheetStatus').value;
			window.location.href='${pageContext.request.contextPath}/timehrses?active='+activeOrAll+'&newTimeSheetStatus='+newTimeSheetStatus;
		});
		
	$(document.body).on('click', '.next-day', function() {
		
		var latestWeekEndDate=$('#datepickerchange').val();
		var date=getDateInFormatMM_DD_YY(latestWeekEndDate);
		var nextWeekEndDate = getNextWeekday(date , 6);
		if(nextWeekEndDate!= null){
		var formattedDate = getFormattedDate(nextWeekEndDate);
		$('#datepickerchange').val(formattedDate);}
		else
		$('#datepickerchange').val(latestWeekEndDate);
		
		$("#datepickerchange").trigger("change");
	});
	
	$(document.body).on('click', '.prev-day', function() {
		
		var latestWeekEndDate=$('#datepickerchange').val();
		var date=getDateInFormatMM_DD_YY(latestWeekEndDate);
		var previousWeekEndDate = getLastWeekday(date , 6);
		var formattedDate = getFormattedDate(previousWeekEndDate);
		$('#datepickerchange').val(formattedDate);
		$("#datepickerchange").trigger("change");
	});
	...
	$(document.body).on('change', '#datepickerchange', function() {
		
		getTimehrsByProjectId();
		
	});
	
	$(document.body).on('change', '#newTimeSheetStatus', function() {
		var newTimeSheetStatus;
		newTimeSheetStatus = document.getElementById('newTimeSheetStatus').value;
		document.getElementById('newTimeSheetStatus').value = newTimeSheetStatus;
		activeOrAll = document.getElementById('activeOrAll').value;
		window.location.href = '${pageContext.request.contextPath}/timehrses?active=' + activeOrAll + '&newTimeSheetStatus=' + newTimeSheetStatus;
	});
	...
	function openMaintainance(id, yashEmpId) {
		...
		document.getElementById("inner").innerHTML = "<b>Select Week End Date:</b>&nbsp;&nbsp;<input type='text' readonly id='datepickerchange' value='"+weekEndDate+"' size='12' align='center'/>&nbsp;&nbsp;<button id='approveAll' disabled>Approve All</button>"
				+ "&nbsp; ";
		getTimehrsByProjectId();
		displayMaintainance();
		timehrsTable = addDataTableSearch($('#timehrsTableId2'));
		...
		$("#weekEndDateSel").datepicker({
		 ...
		 $("#datepickerchange").datepicker({
			 beforeShowDay: function(dt){
				     return [dt.getDay() == 1 || dt.getDay() == 2 || dt.getDay() == 3 || dt.getDay() == 4 || dt.getDay() == 5 || dt.getDay() == 0? false : true];
				 }
	...
	function getTimehrsByProjectId() {
		var projectAllocStatus = "${projectAllocStatus}";
		gProjectId = "${project_Id}";
		gWeekEndDate = $("#datepickerchange").val();
		$.ajax({
			type : 'GET',
			url : '/rms/projects/' + gProjectId,
			success : function(project) {
				gProjectName = project.projectName;
				gProjectEndDate = project.projectEndDate;
				gProjectKickOffDate = project.projectKickOff;
				gProjectId = project.id;

				return;
			}
		});

		 $.getJSON("/rms/timehrses/timehrsByProject?projectId=" + gProjectId + "&timesheetstatus=P&projectAllocStatus=" + projectAllocStatus + "&weekEndDate=" + gWeekEndDate, function(data) {
			showTimehrs(data);
		});
	}
	...
	if (data != '') {
		...
		$("#timehrsTableId2 > tbody:last").append($("#timehrsTableRows").render(data));
		/* timeSheetStatus = document.getElementById('timeSheetStatus').value; */
		timeSheetStatus = "P";
		if (jQuery.isEmptyObject(data))
			$("#inner").find('input[type="button"]').button({
	...
	<div class="tab_seaction">
		...
		<div id='tab1' class="tab_div">
				...
				<div class="search_filter" style="text-align: right;">
					<form id="frmSearch" name="frmSearch" method="post" action=""
						...
						<div class="btnIcon TimeapprvlStatus">
							...
							</select> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <select id='newTimeSheetStatus'>
			...
			<div class="tbl">
				...
				<table width="100%" class="tablesorter fl dataTbl" cellspacing="1"
					id="timehrsTableId1" cellpadding="2">
					<thead>
						<tr>
							...
							<!-- <th width="5%" align="left" valign="middle">Planned Capacity</th>
							<th width="5%" align="left" valign="middle">Actual Capacity</th> -->
							<!-- <th width="11%" align="left" valign="middle">Total Planned Hours</th>
							<th width="12%" align="left" valign="middle">Total Reported Hours</th>
							<th width="10%" align="left" valign="middle">Total Billed Hours</th> -->
							<th width="30%" align="left" valign="middle">Project
								Allocated</th>
						</tr>
					</thead>
					<tbody>
						...
						<c:forEach var="resource" items="${timehrses}">
							...
							<tr id="${resource.employeeId}" class="even">
								...
								<td align="left" valign="middle">${resource.employeeName}</td>
								<td align="left" valign="middle">${resource.designation_name}</td>
								<td align="left" valign="middle">${resource.grade}</td>
								<td align="left" valign="middle"><c:forEach var="projects"
										items="${resource.projectName}" varStatus="loop">
		...
		<div id='tab2' class="tab_div">
			...
			<c:if test="${projectAllocNavigationFlag=='true'}">
				<br>
				<img src="resources/images/arrow_left.gif" />
				<a href="projectallocations" onclick="openProjectAllocation();"
					id="navigationToProjectAlloc"><strong>Back to Project
						Allocation Screen </strong></a>
			</c:if>
			<br> <br>
			<div class="tbl">
...
.notification-text {
    ...
    function openResourceAllocation(){ 
    	...
    	var requestParam = "?empDateOfJoining="+resDateOfJoining+"&"+"empId="+gEmployeeId+"&"+"yashEmpId="+gYashEmployeeId+"&"+"empName="+gEmployeeName;
    	$('#navigationToResAlloc').attr("href", res+requestParam); 
    }
    
function openProjectAllocation(){ 
	var _href = $('#navigationToProjectAlloc').attr('href');
	    	var res = _href.substring(0, 19);
	    	var requestParam = "?projectId="+gProjectId+"&"+"projectKickOffDate="+gProjectKickOffDate+"&"+"projectEndDate="+gProjectEndDate+"&"+"projectName="+gProjectName;
	    	$('#navigationToProjectAlloc').attr("href", res+requestParam);
    }
...
<script>
        var temp="${j}";
	        if(temp==1){ 
	             function myFunction() {
	            	setTimeout(function(){  openMaintainance(temp,temp);}, 2);
				}   
			myFunction();
			
	        }
	        setTimeout(function(){
	        	$("#ToolTables_timehrsTableId2_0").parent().css('display','none');
	        },1000)
