...
@Controller
@RequestMapping("/resourceOnBench")
public class ResourceOnBenchController {

	@Autowired
	@Qualifier("resourceOnBenchService")
	ResourceOnBenchService resourceOnBenchService;
	...
	private static final Logger logger = LoggerFactory.getLogger(ResourceOnBenchController.class);
	...
	public String load(Model uiModel, @RequestParam(value = "empDateOfJoining", required = false) String empDateOfJoining, @RequestParam(value = "empId", required = false) Integer empId,
		...
		return "resourceonbench/list";
	}

	@SuppressWarnings("unchecked")
	@RequestMapping(value = "/list1/{ActiveOrAll}", method = RequestMethod.GET)
	public ResponseEntity<String> list1(Model uiModel, @PathVariable("ActiveOrAll") String activeOrAll, HttpServletRequest request) throws Exception {
		...
		try {
			...
			String[] searchColumns = resourceAllocSearchCriteria.getCols();
			...
			if ((iSortCol != null || iSortCol != "") && (sSortDir != null || sSortDir != "")) {
				...
				resourceAllocSearchCriteria.setOrederBy(resourceAllocSearchCriteria.getCols()[Integer.parseInt(iSortCol)]);

				resourceAllocSearchCriteria.setSortTableName(resourceAllocSearchCriteria.getColandTableNameMap().get(resourceAllocSearchCriteria.getCols()[Integer.parseInt(iSortCol)]));

				resourceAllocSearchCriteria.setRefTableName(resourceAllocSearchCriteria.getColandTableNameMap().get(resourceAllocSearchCriteria.getCols()[Integer.parseInt(iSortCol)]));

				resourceAllocSearchCriteria.setRefColName(resourceAllocSearchCriteria.getCols()[Integer.parseInt(iSortCol)]);
			}
			...
			if (currentLoggedInUserId != null) {
				...
				if (isCurrentUserAdmin) {
					...
					if (isCurrentUserBusinessGroupAdmin) {
						...
						if (search) {
							...
							totalCount = timehrsService.countSearch(resourceAllocSearchCriteria, activeOrAll, buList, null, null, false, true);
						} else

						{
							...
							totalCount = resourceService.findActiveReourceIdsByBusinessGroup(buList, projectIdList, null, false).size();
						}
						...
						if (search) {
							addedResourceList = resourceOnBenchService.findAllocatedResourceEmployeeIdByProjectIdsPagination(page, size, projectIds, activeOrAll, resourceAllocSearchCriteria, true,
									userUtil.getCurrentLoggedInUseId());
							totalCount = resourceOnBenchService.countAllocatedResourceEmployeeIdByProjectIdsPagination(projectIds, activeOrAll, resourceAllocSearchCriteria, true,
									userUtil.getCurrentLoggedInUseId());
						} else {
							addedResourceList = resourceOnBenchService.findAllocatedResourceEmployeeIdByProjectIdsPagination(page, size, projectIds, activeOrAll, resourceAllocSearchCriteria,
									false, userUtil.getCurrentLoggedInUseId());
							totalCount = resourceOnBenchService.countAllocatedResourceEmployeeIdByProjectIdsPagination(projectIds, activeOrAll, resourceAllocSearchCriteria, false,
									userUtil.getCurrentLoggedInUseId());
						}
					...
					if (!isCurrentUserDelManager && !isCurrentUserBusinessGroupAdmin) {
				...
				resultJSON.put("aaData", TimehrsView.toJsonArray(allResources));
			...
			logger.error("RuntimeException occured in list method of ResourceOnBenchController   controller:" + e);
			throw e;
		} catch (Exception e) {
			logger.error("RuntimeException occured in list method of ResourceOnBenchController   controller:" + e);
			throw e;
	...
	public ResponseEntity<String> findResourceAllocationsByResourceEmployeeId(@RequestParam("employeeId") Integer employeeId) throws Exception {
		...
		try {
			...
			resAlloc = resourceOnBenchService.findResourceAllocationsByEmployeeId(resource);
			if (resAlloc.size() != 0) {
				for (org.yash.rms.domain.ResourceAllocation alloc : resAlloc) {
					List<Object[]> lastUserActivity = resourceOnBenchService.findLastUseractivity(alloc.getId());
					/*
					...
					List<Object[]> firstUserActivity = resourceOnBenchService.findFirstUseractivity(alloc.getId());
			...
			logger.error("RuntimeException occured in FindResourceAllocationsByResourceEmployeeId method of ResourceOnBenchController   controller:" + e);
			throw e;
		} catch (Exception exception) {
			logger.error("RuntimeException occured in FindResourceAllocationsByResourceEmployeeId method of ResourceOnBenchController   controller:" + exception);
			throw exception;
		}
	...
	public ResponseEntity<String> createFromJson(@RequestBody String json) throws Exception {
		...
		org.yash.rms.domain.ResourceAllocation resourceAllocation = (org.yash.rms.domain.ResourceAllocation) resourceAllocationMapper.fromJsonToObject(json,
				org.yash.rms.domain.ResourceAllocation.class);
		...
		// being created
		try {
			List<org.yash.rms.domain.ResourceAllocation> allResourceAllocations = resourceOnBenchService.findResourceAllocationsByEmployeeId(resourceAllocation.getEmployeeId());
			List<org.yash.rms.domain.ResourceAllocation> activeResourceAllocations = resourceOnBenchService.findResourceAllocationByEmployeeId(resourceAllocation.getEmployeeId().getEmployeeId());
			...
			for (org.yash.rms.domain.ResourceAllocation allocation : allResourceAllocations) {
				if (!(allocation.getId().equals(resourceAllocation.getId()))) {
					...
					if (allocation.getCurProj() != null && allocation.getCurProj() == true && resourceAllocation.getCurProj() != null && resourceAllocation.getCurProj() == true) {
						...
						resourceOnBenchService.saveOrupdate(allocation);
						/*
			...
			if (flag) {
				if (resourceAllocation.getId() != null) {
					...
					ResourceAllocation resAlloc = resourceOnBenchService.findResourceAllocation(resourceAllocation.getId());
					...
					if(save){
					resourceOnBenchService.saveOrupdate(resourceAllocation);
					}
					//end
			...
			logger.error("RuntimeException occured in createFromJson method of ResourceOnBenchController   controller:" + exception);
			throw exception;
		} catch (Exception e) {
			logger.error("Exception occured in createFromJson method of ResourceOnBenchController   controller:" + e);
			throw e;
		}
	...
	public void updateAllocationSeqForResourceAllocations(Resource employeeId) throws Exception {
		...
		try {
			List<org.yash.rms.domain.ResourceAllocation> resourceAllocation = resourceOnBenchService.findResourceAllocationsByEmployeeId(employeeId);
			if (resourceAllocation != null && !resourceAllocation.isEmpty()) {
				...
				for (int i = 1; i <= resourceAllocation.size(); i++) {
					...
					resourceOnBenchService.saveOrupdate(raObj);

				}

			}
		} catch (RuntimeException e) {
			logger.error("RuntimeException occured in updateAllocationSeqForResourceAllocations method of ResourceOnBenchController   controller:" + e);
			throw e;
		} catch (Exception exception) {
			logger.error("RuntimeException occured in updateAllocationSeqForResourceAllocations method of ResourceOnBenchController   controller:" + exception);
			throw exception;
		}
	...
	public ResponseEntity<String> approveDetails(@PathVariable("id") Integer id, @PathVariable("empId") Integer empId, @PathVariable("projId") Integer projId, HttpServletResponse response)
		...
		try {
			...
			ResourceAllocation resourceAllocation = resourceOnBenchService.findById(id);
			...
			logger.error("RuntimeException occured in approveDetails method of ResourceOnBenchController   controller:" + exception);
			throw exception;
		} catch (Exception e) {
			logger.error("RuntimeException occured in approveDetails method of ResourceOnBenchController   controller:" + e);
			throw e;
		}
	...
	public ResponseEntity<String> deleteFromJson(@PathVariable("id") Integer id) throws Exception {
		...
		HttpHeaders headers = new HttpHeaders();
		try {
			org.yash.rms.domain.ResourceAllocation resourceAllocation = resourceOnBenchService.findResourceAllocation(id);
			...
			resourceOnBenchService.delete(id);
			updateAllocationSeqForResourceAllocations(employeeId);
		} catch (RuntimeException exception) {
			logger.error("RuntimeException occured in deleteFromJson method of ResourceOnBenchController   controller:" + exception);
			throw exception;
		} catch (Exception e) {
			logger.error("RuntimeException occured in deleteFromJson method of ResourceOnBenchController   controller:" + e);
			throw e;
		}
	...
	public String copyFromJson(@PathVariable("id") Integer id, @PathVariable("selectedResourceIds") String ids) throws Exception {
		...
		try {
			...
			allCopied = resourceOnBenchService.getBenchResource(resourceAllocation, ids, resourceOnBenchService, resourceService);
		} catch (RuntimeException exception) {
			logger.error("RuntimeException occured in copyFromJson method of ResourceOnBenchController   controller:" + exception);
			throw exception;
		} catch (Exception e) {
			logger.error("RuntimeException occured in copyFromJson method of ResourceOnBenchController   controller:" + e);
			throw e;
		}
	...
	public ResponseEntity<String> findResourcesByActiveType(@RequestParam("employeeId") Integer employeeId) throws Exception {
		...
		try {
			...
			resAlloc = resourceOnBenchService.findResourceAllocationByActiveTypeEmployeeId(resource);
			...
			logger.error("RuntimeException occured in findResourcesByActiveType method of ResourceOnBenchController   controller:" + e);
			throw e;
		} catch (Exception exception) {
			logger.error("RuntimeException occured in findResourcesByActiveType method of ResourceOnBenchController   controller:" + exception);
			throw exception;
		}
	...
	public String employeeDetails(@PathVariable("employeeid") Integer employeeId, Model uiModel) throws Exception {
		...
		try {
			...
			logger.error("RuntimeException occured in employeeDetails method of ResourceOnBenchController  controller:" + runtimeException);
		...
		return "resourceonbench/readonlyEmployeeDetails";
	}
	...
	public ResponseEntity<String> getAllocations(@PathVariable("selectedprojectId") Integer projId) throws Exception {
		...
		for (ResourceAllocation resourceAllocation : resourceOnBenchService.getAllocations(projId)) {
	...
	public ResponseEntity<String> checkForTimesheetGreaterThanAllocEndDate(@PathVariable("resAllocId") Integer resourceAllocId, @PathVariable("weekEndDate") Date allocWeekEndDate,
		...
		List<UserActivity> userActivityList = resourceOnBenchService.isFutureTimesheetpresent(resourceAllocId, allocWeekEndDate);
	...
	public ResponseEntity<String> deleteFutureTimesheet(@RequestParam(value = "userActivityId") String userActivityId, @RequestParam(value = "resourceAllocId") String resourceAllocId,
		...
		resourceOnBenchService.deleteFutureTimesheet(userActivityIdList, resourceAllocId, weekEndDate);
	...
	public ResponseEntity<String> checkIfAllocationIsOpen(@PathVariable("resourceAllocId") Integer resourceAllocId, @PathVariable("weekDate") Date weekDate, HttpServletResponse response)
		...
		boolean status = resourceOnBenchService.checkIfAllocationIsOpen(resourceAllocId, weekDate);
		if (status == true) {
			return new ResponseEntity<String>("{ \"status\":\"true\"}", headers, HttpStatus.OK);
		}
	...
	public ResponseEntity<String> getDownloadResumeAndTEFfiles(HttpServletRequest request, @RequestParam("id") Integer employeeId, @RequestParam("downloadFileFlag") String downloadFileFlag, HttpServletResponse response) {
	...
	public ResponseEntity<String> list(Model uiModel, @PathVariable("ActiveOrAll") String activeOrAll, HttpServletRequest request) throws Exception {
		...
		try {
			...
			Integer iColumns = Integer.parseInt(request.getParameter("iColumns"));
			String sSearch = request.getParameter("sSearch");
			ResourceOnBenchSearchCriteria searchCriteria = new ResourceOnBenchSearchCriteria();
			String[] searchColumns = searchCriteria.getCols();
			...
			if ((iSortCol != null || iSortCol != "") && (sSortDir != null || sSortDir != "")) {
				searchCriteria.setOrderStyle(sSortDir);
				searchCriteria.setOrederBy(searchCriteria.getCols()[Integer.parseInt(iSortCol)]);

				searchCriteria.setSortTableName(searchCriteria.getColandTableNameMap().get(searchCriteria.getCols()[Integer.parseInt(iSortCol)]));

				searchCriteria.setRefTableName(searchCriteria.getColandTableNameMap().get(searchCriteria.getCols()[Integer.parseInt(iSortCol)]));

				searchCriteria.setRefColName(searchCriteria.getCols()[Integer.parseInt(iSortCol)]);
			}
			...
			for (int i = 0; i < iColumns; i++) {
				...
				if (sSearch.length() > 0) {
					search = true;
					columnName = searchColumns[i];
					searchCriteria.setRefTableName(searchCriteria.getColandTableNameMap().get(columnName));
					searchCriteria.setRefColName(columnName);
					searchCriteria.setSearchValue(sSearch);
					break;
				}
			}
			if (currentLoggedInUserId != null) {
				List<ResourceOnBenchDto> resourceList=null;
				
				resourceList = resourceOnBenchService.resourceAllocationPagination(page, size, searchCriteria, activeOrAll, search);
				resultJSON.put("aaData", ResourceOnBenchDto.toJsonArray(resourceList));
				Integer resourceCount = resourceOnBenchService.getResourceCount(page, size, searchCriteria, activeOrAll, search);
				resultJSON.put("iTotalRecords", resourceCount);
				resultJSON.put("iTotalDisplayRecords", resourceCount); 
			}
	...
	public List<TimehrsViewDTO> convertTimehrsViewDomainToDTOList(List<ResourceOnBenchDto> resourceList) {
		List<TimehrsViewDTO> allResources = new ArrayList<TimehrsViewDTO>();
		Iterator iterator = resourceList.iterator();
		while (iterator.hasNext()) {

			ResourceOnBenchDto obj =  (ResourceOnBenchDto) iterator.next();
			TimehrsViewDTO view = new TimehrsViewDTO();
				...
				view.setDesignation_name(obj.getDesignation_name());
				view.setGrade("");
			view.setDateOfJoining(obj.getDateOfJoining());
			view.setCurrentProject("");
	...
	@RequestMapping(params = "find=byAllocStartDateAndResourceEmployeeId", headers = "Accept=application/json")
	@ResponseBody
	public ResponseEntity<String> findResourcesByAllocStartDateAndResourceEmployeeId(@RequestParam("employeeId") Integer employeeId, @RequestParam("allocationStartDate") java.sql.Date allocationStartDate) throws Exception {
		logger.info("------ResourceAllocationController findResourcesByAllocStartDateAndResourceEmployeeId method start------");
		HttpHeaders headers;
		List<org.yash.rms.domain.ResourceAllocation> resAlloc = null;
		headers = new HttpHeaders();
		try {
			...
			resAlloc = resourceOnBenchService.findResourceAllocationByActiveTypeEmployeeId(resource, allocationStartDate);
			...
			logger.error("RuntimeException occured in findResourcesByAllocStartDateAndResourceEmployeeId method of ResourceOnBenchController   controller:" + e);
			throw e;
		} catch (Exception exception) {
			logger.error("RuntimeException occured in findResourcesByAllocStartDateAndResourceEmployeeId method of ResourceOnBenchController   controller:" + exception);
			throw exception;
		}

		logger.info("------ResourceAllocationController findResourcesByAllocStartDateAndResourceEmployeeId method end------");
		if (allCopied)
			return new ResponseEntity<String>(resourceAllocationMapper.toJsonArray(resAlloc), headers, HttpStatus.OK);
		else {
			allCopied = true;
