...
public class LocationController {
	...
	@Autowired
	JsonObjectMapper<Location> jsonMapper;
	...
	public String list(@RequestParam(value = "page", required = false) Integer page,@RequestParam(value = "size", required = false) Integer size,Model uiModel) throws Exception {
		...
		logger.info("------locationController list method start------");
		try{
		if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
            uiModel.addAttribute(Constants.LOCATION, locationService.findByEntries(firstResult, sizeNo));
            float nrOfPages = (float) locationService.countTotal() / sizeNo;
		...
		{				
			logger.error("RuntimeException occured in list method of location controller:"+runtimeException);				
			throw runtimeException;
		}catch(Exception exception){
			logger.error("Exception occured in list method of location controller:"+exception);				
			throw exception;
		}
		logger.info("------locationController list method end------");
	...
	public ResponseEntity<String> updateFromJson(@RequestBody String json,HttpServletRequest request) throws Exception {
		logger.info("------locationController updateFromJson method start------");
		...
		// Update the 
		try{
			Location location = jsonMapper.fromJsonToObject(json,Location.class);
			location.setLastUpdatedId((String)context.getAttribute("userName"));
			
		if (!locationService.saveOrupdate(location)) {
			return new ResponseEntity<String>("{ \"status\":\"FALSE\"}",headers, HttpStatus.NOT_FOUND);
		}
		}catch(RuntimeException runtimeException)
		{				
			logger.error("RuntimeException occured in updateFromJson method of location controller:"+runtimeException);				
			throw runtimeException;
		}catch(Exception exception){
			logger.error("Exception occured in updateFromJson method of location controller:"+exception);				
			throw exception;
		}  
		logger.info("------locationController updateFromJson method end------");
    ...
    public ResponseEntity<String> deleteFromJson(@PathVariable("id") Integer id) throws Exception {
    	logger.info("------locationController deleteFromJson method start------");
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json");
        //Delete based on ID
        try {
			
        if (!locationService.delete(id.intValue())) {
            return new ResponseEntity<String>("{ \"status\":\"FALSE\"}",headers, HttpStatus.NOT_FOUND);
        }
        
        } catch(RuntimeException runtimeException)
		{				
			logger.error("RuntimeException occured in deleteFromJson method of location controller:"+runtimeException);				
			return new ResponseEntity<String>("{ \"status\":\"FALSE\"}",headers, HttpStatus.EXPECTATION_FAILED);
//			throw runtimeException;
		}catch(Exception exception){
			logger.error("Exception occured in deleteFromJson method of location controller:"+exception);				
			return new ResponseEntity<String>("{ \"status\":\"FALSE\"}",headers, HttpStatus.EXPECTATION_FAILED);
//			throw exception;
		}  
		logger.info("------locationController deleteFromJson method end------");
        return new ResponseEntity<String>("{ \"status\":\"TRUE\"}",headers, HttpStatus.OK);
    }
    ...
    public ResponseEntity<String> createFromJson(@RequestBody String json,HttpServletRequest request) throws Exception {
    	logger.info("------locationController createFromJson method start------");
    	ServletContext context=request.getSession().getServletContext();
    	context.getAttribute("userName");
    	//Convert JSon Object to Domain Object
    	Location location = jsonMapper.fromJsonToObject(json, Location.class);
    	location.setCreatedId((String)context.getAttribute("userName"));
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json");
        try{
        if(!locationService.saveOrupdate(location)){
		...
		{				
			logger.error("RuntimeException occured in createFromJson method of location controller:"+runtimeException);				
			throw runtimeException;
		}catch(Exception exception){
			logger.error("Exception occured in createFromJson method of location controller:"+exception);				
			throw exception;
		}  
		logger.info("------locationController createFromJson method end------");
